...
export function DeskLabOrdersList({
  ...
  onNotDone?: (orderId: string, reasonCode: string, reasonNote?: string) => void;
  ...
  if (loading) {
    return (
      <p className="text-sm text-muted-foreground" data-testid="desk-lab-orders-loading">
        Loading tests…
      </p>
    );
  }
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-sm text-foreground">
                  ...
                  {order.kind ? (
                    <span className="text-muted-foreground"> · {order.kind}</span>
                  ) : null}
                </p>
                {order.status === "uploaded" ? (
                  <p className="text-xs text-muted-foreground">On this report</p>
                ) : null}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {LAB_NOT_DONE_REASONS.map((reason) => (
                    <Button
                      key={reason.code}
                      type="button"
                      variant={reason.code === "other" ? "outline" : "secondary"}
                      size="sm"
                      className="h-7"
                      disabled={busy}
                      onClick={() => {
...
export function DeskDocumentsStrip({
  ...
  const ordersQuery = useQuery({
    ...
    enabled: mode === "internal_labs" && Boolean(token) && Boolean(appointmentId),
  });
  const title = mode === "internal_labs" ? "Upload reports" : "Patient files";
  const hint =
    mode === "internal_labs"
      ? "Photograph the report, then mark each test this file covers. If a test was not run, say why."
  ...
  useEffect(() => {
    ...
    void listDeskVisitDocuments(token, appointmentId)
      ...
      .catch((err) => {
        if (!cancelled) setError(deskErrorMessage(err, "Could not load reports"));
      })
      .finally(() => {
  ...
  async function uploadFiles(fileList: FileList | null) {
    ...
    const remaining = VISIT_DOCUMENT_MAX_PAGES - pageCount(documents);
    if (remaining <= 0) {
    ...
    try {
      ...
      let activeId = activeDocumentIdRef.current;
      let accepted = 0;
      for (let i = 0; i < fileList.length && accepted < remaining; i++) {
        ...
        const uploadRes = await getDeskVisitDocumentUploadUrl(token, appointmentId, {
          filename: file.name,
      ...
      if (mode === "internal_labs" && accepted > 0) {
  ...
  async function writeLabOrders(updates: Parameters<typeof upsertDeskLabOrders>[2]) {
    setBusy(true);
    setError(null);
    try {
  ...
  const extractGroup = useCallback(
    async (
      ...
      { ok: true; group: LabExtractVerifyGroup } | { ok: false; message: string | null }
    > => {
      try {
        ...
        if (res.data.source === "vision" && page.file_type.startsWith("image/")) {
          try {
  ...
  async function startExtractPages(doc: VisitDocument, pages: VisitDocumentPage[]) {
    ...
    if (next.length === 0) {
      setError(firstFailure ?? "Could not extract this report. Enter the values on the visit.");
      return;
    }
    if (failed > 0) {
        ...
        >
          <div className="flex flex-wrap items-end gap-2">
            ...
            <div className="min-w-[7rem]">
              ...
              {mode === "internal_labs" ? (
                <p className="mt-1 flex h-9 items-center text-sm text-foreground">Us</p>
              ) : (
                <p className="mt-1 flex h-9 items-center text-sm text-foreground">
                  Outside
                </p>
                ...
                <div key={page.id} className="flex flex-col items-stretch">
                  ...
                  {canExtract && isExtractableVisitPage(page) ? (
                    <button
                      ...
                      disabled={busy || extractBusy || extracted || verifyOpen}
                      data-testid={
                        extracted ? "desk-page-lab-extract-done" : "desk-page-lab-extract"
                      }
                    >
