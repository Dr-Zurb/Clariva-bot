...
import type { CvsExamChipGroupDef } from "@/lib/cockpit/cvs-exam-finding-schema";
import {
...
export interface ExamCvsChipGroupCardProps {
  group: CvsExamChipGroupDef;
...
export function ExamCvsChipGroupCard({
  group,
  subsectionLabel,
  entries,
  disabled = false,
  ...
  return (
    <article
      ...
      {...{ [EXAM_CVS_FINDING_CARD_ATTR]: cardId }}
      data-testid={`cvs-chip-group-card-${cardId}`}
    ...
    >
      <div className="flex items-center gap-2 px-1.5 py-1.5">
        <button
          ...
          data-testid={`cvs-chip-group-toggle-${cardId}`}
        >
          {isRecorded && !open ? (
        ...
        {isRecorded ? (
          <button
            ...
            data-testid={`cvs-chip-group-clear-${cardId}`}
      ...
      <Collapse open={open} className="space-y-2 border-t border-border/50 px-2 pb-2 pt-2">
          <div
            ...
            role="group"
            aria-label={`Cardiovascular — ${subsectionLabel} — ${group.label}`}
            data-testid={`cvs-chip-group-${cardId}`}
          >
          ...
          {notesFindingId ? (
            <div>
              <label htmlFor={`cvs-chip-group-notes-${cardId}`} className={RX_EXAM_FIELD_LABEL_BLOCK_CLASS}>
                Notes
              </label>
              <input
                id={`cvs-chip-group-notes-${cardId}`}
                type="text"
                value={notes}
                disabled={disabled}
                ...
                data-testid={`cvs-chip-group-notes-${cardId}`}
              />
            </div>
          ) : null}
          <div className="flex justify-center pt-0.5">
            <button
              ...
              data-testid={`cvs-chip-group-done-${cardId}`}
...
export function clearCvsChipGroupEntries(
  entries: ExamFindingEntry[],
  group: CvsExamChipGroupDef,
): ExamFindingEntry[] {
...
export function patchCvsChipGroupNotesEntry(
  entries: ExamFindingEntry[],
