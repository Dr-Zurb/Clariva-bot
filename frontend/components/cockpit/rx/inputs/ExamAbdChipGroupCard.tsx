...
import type { AbdExamChipGroupDef } from "@/lib/cockpit/abd-exam-finding-schema";
import {
  abdChipGroupCardId,
...
export interface ExamAbdChipGroupCardProps {
  group: AbdExamChipGroupDef;
...
export function ExamAbdChipGroupCard({
  group,
  subsectionLabel,
  entries,
  disabled = false,
    ...
    >
      <div className="flex items-center gap-2 px-1.5 py-1.5">
        <button
          ...
          data-testid={`abd-chip-group-toggle-${cardId}`}
        >
          {isRecorded && !open ? (
        ...
        {isRecorded ? (
          <button
            ...
            data-testid={`abd-chip-group-clear-${cardId}`}
      ...
      <Collapse open={open} className="space-y-2 border-t border-border/50 px-2 pb-2 pt-2">
          <div
            ...
            role="group"
            aria-label={`Abdomen — ${subsectionLabel} — ${group.label}`}
            data-testid={`abd-chip-group-${cardId}`}
          >
          ...
          {notesFindingId ? (
            <div>
              <label htmlFor={`abd-chip-group-notes-${cardId}`} className={RX_EXAM_FIELD_LABEL_BLOCK_CLASS}>
                Notes
              </label>
              <input
                id={`abd-chip-group-notes-${cardId}`}
                type="text"
                value={notes}
                disabled={disabled}
                ...
                data-testid={`abd-chip-group-notes-${cardId}`}
              />
            </div>
          ) : null}
          <div className="flex justify-center pt-0.5">
            <button
              ...
              data-testid={`abd-chip-group-done-${cardId}`}
...
export function clearAbdChipGroupEntries(
  entries: ExamFindingEntry[],
  group: AbdExamChipGroupDef,
): ExamFindingEntry[] {
...
export function patchAbdChipGroupNotesEntry(
  entries: ExamFindingEntry[],
