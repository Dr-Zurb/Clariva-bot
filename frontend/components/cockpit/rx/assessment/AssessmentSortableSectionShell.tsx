...
export interface AssessmentSectionDragHandleProps {
...
/** Six-dot reorder grip for Assessment L1 `CollapsibleContainer.leadingActions`. */
export function AssessmentSectionDragHandle({
  dragHandleProps,
  ariaLabel,
  disabled = false,
  index,
  ...
  onMoveUp,
  onMoveDown,
}: AssessmentSectionDragHandleProps) {
  return (
    <div
      {...dragHandleProps}
      role="button"
      ...
      data-testid="assessment-section-drag-handle"
...
export interface AssessmentSortableSectionShellProps {
...
/** Drop-target wrapper for one Assessment L1 section block. */
export function AssessmentSortableSectionShell({
  sectionId,
  disabled = false,
  children,
  dropIntent = null,
  ...
  onDragLeave,
  onDrop,
}: AssessmentSortableSectionShellProps) {
  return (
    <div
      className="relative"
      data-assessment-section-id={sectionId}
