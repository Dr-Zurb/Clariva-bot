...
import { isSectionHidden } from "@/lib/cockpit/assessment-section-visibility";
...
export function resolveAssessmentSectionHasDataHint(
  ...
  switch (sectionId) {
    ...
    case "known_conditions":
      // Chart-backed — manage menu never claims form data for this L1.
      return false;
...
export interface ManageAssessmentSectionsMenuProps {
  disabled?: boolean;
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  sectionOrder: readonly AssessmentSectionId[];
  mountableIds: readonly AssessmentSectionId[];
  hiddenIds: readonly string[];
  fields: RxFormFields;
  customSections: readonly CustomSubsection[];
  onToggleHidden: (sectionId: AssessmentSectionId) => void;
  onMoveSection: (sectionId: AssessmentSectionId, direction: "up" | "down") => void;
  onAddCustomSection: () => void;
  onRemoveCustomSection: (sectionId: AssessmentSectionId) => void;
}

export function ManageAssessmentSectionsMenu({
  disabled = false,
  open,
  onOpenChange,
  sectionOrder,
...
}: ManageAssessmentSectionsMenuProps) {
  ...
  const hiddenMountableCount = useMemo(
    () =>
      menuSections.filter((id) => isSectionHidden(id, hiddenIds, mountableIds))
        .length,
        ...
        <IconTooltip label={triggerLabel}>
          <PopoverTrigger asChild>
            <Button
              ...
              data-testid="assessment-section-manager-trigger"
            >
      ...
      <PopoverContent align="end" className="w-[30rem] max-w-[calc(100vw-2rem)] p-0">
        ...
        <ul className="max-h-80 overflow-y-auto p-1.5" role="list">
          {menuSections.map((sectionId, menuIndex) => {
            const label = resolveAssessmentSectionLabel(sectionId, customSections);
            const hasData = resolveAssessmentSectionHasDataHint(sectionId, fields, customSections);
            ...
            return (
              <li
                ...
                data-testid={`assessment-section-manager-row-${sectionId}`}
              >
                ...
                <div className="flex shrink-0 items-center gap-0.5">
                  {isCustom ? (
                    <RemoveIconButton
                      label={`Remove ${label}`}
                      disabled={disabled}
                      testId={`assessment-section-manager-remove-${sectionId}`}
                      className="h-8 w-8"
                  ...
                  <button
                    ...
                    aria-pressed={hidden}
                    disabled={disabled}
                    data-testid={`assessment-section-manager-toggle-${sectionId}`}
        ...
        <div className="border-t border-border p-2">
          <button
            ...
            disabled={disabled}
            data-testid="assessment-section-manager-add-custom"
