...
import TranscriptReplayPlaceholder from "./TranscriptReplayPlaceholder";
...
export default function ConsultArtifactsPanel(
  ...
  return (
    ...
    >
      ...
      {/* Sub-batch D · task-video-D3 — doctor-only snapshot review +
          attach surface. Backend rejects non-doctor callers; we
          additionally don't mount on the patient-facing variant so
          the patient never even fetches the gallery. */}
      {callerRole === "doctor" ? (
        <SnapshotReviewPanel sessionId={sessionId} doctorJwt={token} />
      ) : null}

      {/* Sub-batch D · task-video-D2 — placeholder for the click-to-seek
          transcript timeline. Full feature gated on Plan 10 (chunked
          transcript replay endpoint). Renders independently of Plan 10
          so the layout stays stable when Plan 10 lands. */}
      <TranscriptReplayPlaceholder />
