...
function formatDate(iso: string): string {
  // The original call passed `timeStyle: "short"` but invoked
  // `toLocaleDateString`, which silently ignores it — so the rendered
  // output was always a medium date. Preserve that.
...
export default function PreviousPrescriptions({
  ...
  const reload = useCallback(async () => {
    try {
      ...
      // Keep the previous list visible on transient failure rather
      // than blanking the surface — a Resend kebab action that
      // succeeded shouldn't visually wipe past Rx because the
      // refresh failed.
    }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await listPrescriptionsByPatient(token, patientId);
        const list = res.data.prescriptions ?? [];
        if (!cancelled) setPrescriptions(list.slice(0, limit));
      } catch {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link
          href={`/dashboard/patients-v2/${patientId}#prescriptions`}
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  {rx.sent_to_patient_at && (
                    <span
                      className="ml-2 inline-block rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700"
                      title={`Sent to patient at ${formatDate(rx.sent_to_patient_at)}`}
                    >
                      Sent
                    </span>
                  )}
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="text-sm font-medium text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded px-1"
                  >
                    {isExpanded ? "Hide" : "View"}
                  </button>
                  {/*
                    EHR Sub-batch B2 / T3.19 — kebab is gated on
                    `sent_to_patient_at` because the actions only
                    make sense once the Rx has been delivered at
                    least once (Resend / Regenerate PDF / Copy
                    share link all assume an existing artifact +
                    audience). Before first send the doctor's
                    primary surface is the in-form "Send to
                    patient" CTA.
                  */}
                  {rx.sent_to_patient_at && (
                    <PrescriptionPastActionsKebab
                      prescriptionId={rx.id}
                      token={token}
                      onResendSuccess={reload}
                    />
                  )}
                </div>
              </div>
