...
/**
 * All prescriptions for a patient. Shown on patient detail page.
 * @see e-task-6
 */
export default function PatientPrescriptions({ patientId, token }: PatientPrescriptionsProps) {
  ...
  return (
    <div id="prescriptions" className="mt-6 rounded-lg border border-gray-200 bg-gray-50 p-4">
      <h2 className="mb-3 text-lg font-semibold text-gray-900">Prescriptions</h2>
      {loading ? (
        <p className="text-sm text-gray-500">Loading…</p>
      ) : prescriptions.length === 0 ? (
        <p className="text-sm text-gray-500">No prescriptions.</p>
      ) : (
        <ul className="space-y-2" aria-label="Prescriptions">
          {prescriptions.map((rx) => {
            const isExpanded = expandedId === rx.id;
            const diagnosis = rx.provisional_diagnosis
              ? rx.provisional_diagnosis.slice(0, 60) +
                (rx.provisional_diagnosis.length > 60 ? "…" : "")
              : null;
            return (
              <li key={rx.id} className="rounded border border-gray-200 bg-white p-3 text-sm">
