...
import type { DoctorSettings, ModeSchedule, OpdMode, PatchDoctorSettingsPayload } from "@/types/doctor-settings";
...
export default function OpdModePage() {
  ...
  const [modeScheduleSaveError, setModeScheduleSaveError] = useState<string | null>(null);
  const [savedModeSchedule, setSavedModeSchedule] = useState<ModeSchedule>({});
  ...
  const applySettings = useCallback((s: DoctorSettings) => {
    ...
    setSavedModeSchedule(modeScheduleFromPolicies(s.opd_policies));
  }, []);
  ...
  const handleSaveModeSchedule = async (schedule: ModeSchedule, mirroredOpdMode?: OpdMode) => {
    ...
    try {
      ...
      const res = await patchDoctorSettings(authToken, payload);
      applySettings(res.data.settings);
    } catch (err) {
      setModeScheduleSaveError(err instanceof Error ? err.message : "Save failed");
    } finally {
      ...
      {token && (
        <div className="mt-8">
          <ModeScheduleEditor
            ...
            currentOpdModeColumn={defaultOpdMode(settings)}
