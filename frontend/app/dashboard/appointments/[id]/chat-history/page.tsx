...
interface PageProps {
  ...
  searchParams: Promise<{ from?: string; pid?: string; date?: string }>;
}

export default async function DoctorChatHistoryPage({
  params,
  searchParams,
}: PageProps) {
  const { id } = await params;
  const query = await searchParams;
  const origin = parseCockpitOrigin(query.from ?? null);
  const patientId = query.pid ?? null;
  const opdDate = query.date ?? null;
  const backTarget = resolveBackTarget(origin, patientId, opdDate);
  const originParams = new URLSearchParams();
  if (query.from) originParams.set("from", query.from);
  if (query.pid) originParams.set("pid", query.pid);
  if (query.date) originParams.set("date", query.date);
  const appointmentBackHref = buildCockpitAppointmentPathFromCurrentOrigin(
    id,
    originParams,
  );
