...
export function useModalityUpgradeFSM(
  ...
  useEffect(() => {
    if (!open) return;
    if (!sessionId) return;
    // Per-mount nonce — see useVideoEscalationState.ts for the rationale.
    // tl;dr: `@supabase/ssr`'s singleton client + React Strict Mode would
    // otherwise hand back the previously-subscribed channel object on
    // re-mount, and `.on()` after `.subscribe()` throws.
    const client = createClient();
    const nonce =
    ...
    const channel = client
      .channel(`modality-upgrade:${sessionId}:${nonce}`)
      .on(
        "postgres_changes",
        {
          event: "UPDATE",
