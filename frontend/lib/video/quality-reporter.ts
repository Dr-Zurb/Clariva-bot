/**
 * Video Call Quality Reporter — Sub-batch E · task-video-E6.
 *
 ...
 * `POST /api/v1/consultation/:sessionId/video-quality` endpoint
 * (Migration 086 + `video-call-quality-service.ts`).
 *
 * Cadence (decision §26 — same as voice C2 sibling)
 * --------------------------------------------------
 * The first 60 seconds of a call are when the user is most likely to
 * notice problems — they're switching networks, the SDK is warming up
 * the encoder, and the patient is leaning into the camera. We sample
 * EVERY 10 SECONDS for the first minute (6 samples) so analytics
 * captures the warmup curve. After 60s, we relax to 30s cadence
 * (~58 samples in the next 29min). Total per side per 30-min call ≈
 * 64 samples, ≈ 128 across both sides — well under the 256 / batch cap.
 *
 * Buffering + flush
 * -----------------
 ...
 *   - Video metrics (resolution, fps, bitrates)
 *   - Twilio room SID (for cross-referencing with their composition logs)
 *
 * NO transcript content, NO message bodies, NO identifiers beyond what
 * the backend already correlates via the sessionId path param. Spec
 * §"PHI hygiene": "no transcript content; only acoustic + network +
 * video metrics."
 *
 * @see frontend/lib/video/twilio-stats-parse.ts (the shared parsers)
 * @see frontend/lib/api.ts (postVideoQualitySamples — the POST helper)
 * @see backend/src/services/video-call-quality-service.ts (the ingest endpoint)
 * @see frontend/components/consultation/VideoRoom.tsx (the mount site)
 */
...
export type QualityReporterRole = "doctor" | "patient";

/**
 * One QoS sample. Field names mirror the column names in
 * `video_call_quality` table (snake_case → camelCase). Backend
...
export type SamplePoster = (samples: VideoQualitySample[]) => Promise<unknown>;

export interface CreateQualityReporterOptions {
  /** Twilio Room — the source of all stats. Reporter no-ops if null. */
  room: Room;
   ...
   * `lib/api.ts#postVideoQualitySamples` is the canonical implementation.
   */
  poster: SamplePoster;
}

export interface VideoQualityReporter {
...
const FAST_CADENCE_MS = 10_000;
/** Then 30s thereafter. */
const SLOW_CADENCE_MS = 30_000;
...
// ============================================================================
// Internal — byte-delta state for kbps computation
// ============================================================================
...
const EMPTY_PREVIOUS: PreviousByteSample = {
  bytesSent: null,
  bytesReceived: null,
  timestampMs: 0,
};
...
/**
 * Create a QoS reporter bound to a live Twilio room. Caller is
 * responsible for invoking `dispose()` on call teardown (the reporter
 * doesn't subscribe to room events — too many edge cases with
 * reconnects; the parent `<VideoRoom>` already owns the lifecycle).
 *
 * Sampling starts immediately (first sample at t=0) and continues on
 * the cadence schedule until `dispose()` is called.
 */
export function createVideoQualityReporter(
  options: CreateQualityReporterOptions,
): VideoQualityReporter {
  const { room, sessionId, role, poster } = options;

  let disposed = false;
  let sampleSeq = 0;
  const buffer: VideoQualitySample[] = [];
  let prevBytes: PreviousByteSample = EMPTY_PREVIOUS;
  ...
  function nextCadenceMs(): number {
    ...
    return elapsed < FAST_PHASE_DURATION_MS ? FAST_CADENCE_MS : SLOW_CADENCE_MS;
  }

  /**
   * Take one sample. Reads `room.getStats()` + the local participant's
   * `networkQualityLevel`, parses through the shared module, computes
   * kbps via byte-delta from the previous sample, pushes onto the
   * buffer.
   *
  ...
  async function takeSample(): Promise<void> {
    ...
    try {
      ...
      if (!report) {
        // No peer connection yet (pre-connect); skip but still increment
        // the seq so the gap is visible in analytics.
        sampleSeq += 1;
        return;
      }
      ...
      const deltaMs = prevBytes.timestampMs > 0 ? nowMs - prevBytes.timestampMs : 0;
      const kbpsSend = computeKbps(currentBytesSent, prevBytes.bytesSent, deltaMs);
      ...
      const resolution = readResolution(report);
      ...
      const sample: VideoQualitySample = {
        sampleSeq: sampleSeq++,
        networkQualityLevel,
        ...
        videoResolutionW: resolution?.width ?? null,
        videoResolutionH: resolution?.height ?? null,
        videoFps: readFps(report),
        kbpsSend,
        kbpsReceive,
  ...
  /**
   * Flush the buffer to the backend. Snapshots the current buffer
   * (so a sample taken mid-POST goes into the next batch instead of
   * being lost), POSTs, and clears the snapshotted slice on success.
   * On failure, restores the snapshot to the front of the buffer so
   * the next flush retries (subject to the high-watermark cap).
   */
  ...
  // first batch picks up the 6 fast-phase samples + the first 30s-cadence
  // sample, ~7 samples. Cheap, and aligns batches with the cadence
  // boundary so per-batch sample counts are predictable.
  ...
  return {
    ...
    dispose: () => {
      ...
      // they're forwarded into the poster closure at construction time
      // (the poster is bound by the caller — see lib/api.ts).
      void role;
      void sessionId;
    },
  };
