 ...
 * Keep in sync with `backend/src/utils/advice-format.ts`.
 */
...
/** Hydrate form advice from both columns (education folded in). */
export function hydrateAdviceField(
  advice: string | null | undefined,
  patientEducation: string | null | undefined,
): string {
  return resolveAdviceForOutput(advice, patientEducation) ?? "";
}
