...
test.describe("Dashboard access", () => {
  test("unauthenticated visit to dashboard OPD redirects to login", async ({
    page,
  }) => {
    await page.context().clearCookies();
    await page.goto("/dashboard/opd-today", { waitUntil: "domcontentloaded" });
    await expect(page).toHaveURL(/\/(login|auth)/, { timeout: 10_000 });
  });

  test("legacy /dashboard/appointments redirects to OPD today", async ({
    page,
  }) => {
    await page.context().clearCookies();
    await page.goto("/dashboard/appointments", { waitUntil: "domcontentloaded" });
    await expect(page).toHaveURL(/\/(login|auth|dashboard\/opd-today)/, {
      timeout: 10_000,
    });
  });
});
...
test.describe("Full flow: login → dashboard → OPD", () => {
  ...
  test("login, navigate to dashboard, then OPD today", async ({
    page,
  }) => {
    ...
    await page.getByRole("link", { name: /^opd$/i }).click();
    await expect(page).toHaveURL(/\/dashboard\/opd-today/);
  });
});
