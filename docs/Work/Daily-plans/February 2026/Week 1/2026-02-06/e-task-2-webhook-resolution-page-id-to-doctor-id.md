...
- [WEBHOOKS.md](../../Reference/engineering/operations/WEBHOOKS.md) - Retry, dead letter, idempotency (Instagram: `entry[0].id`), audit format
- [COMPLIANCE.md](../../Reference/engineering/compliance/COMPLIANCE.md) - No PHI in logs; audit
- [ARCHITECTURE.md](../../Reference/engineering/architecture/ARCHITECTURE.md) - Service layer for lookup
- [STANDARDS.md](../../Reference/engineering/development/STANDARDS.md) - Logging, PII redaction
- [TESTING.md](../../Reference/engineering/development/TESTING.md) - Unit/integration test patterns
- [DEFINITION_OF_DONE.md](../../Reference/engineering/development/DEFINITION_OF_DONE.md) - Completion checklist
- [RECIPES.md](../../Reference/engineering/development/RECIPES.md) - Implementation patterns (service/worker)
...
### 3. Worker changes
- [x] ✅ 3.1 Before using doctorId: get page_id from payload; call the new resolution function with page_id; if result is null (unknown page): do not create a conversation; optionally send a fallback reply to the user; mark webhook as failed in idempotency (e.g. status `failed`); write audit log (action e.g. `webhook_processed`, status failure, metadata: page_id or hash only, no PHI); return without processing. Do not dead-letter for unknown page (retry would not succeed); dead-letter is for transient failures per [WEBHOOKS.md](../../Reference/engineering/operations/WEBHOOKS.md). - **Completed: 2026-02-06**
...
- [x] ✅ 4.1 Audit log when page_id has no linked doctor: use `audit_logs` with `resource_type='webhook'`, action e.g. `webhook_processed`, status failure, metadata (event_id, provider, page_id or hash only—no PHI) per [WEBHOOKS.md](../../Reference/engineering/operations/WEBHOOKS.md) and [COMPLIANCE.md](../../Reference/engineering/compliance/COMPLIANCE.md). - **Completed: 2026-02-06**
- [x] ✅ 4.2 No token or PHI in logs (STANDARDS.md, COMPLIANCE.md). - **Completed: 2026-02-06**

### 5. Verification
- [x] ✅ 5.1 Unit test for the resolution function: returns doctor_id when a row exists for the page_id, returns null when no row exists. Follow [TESTING.md](../../Reference/engineering/development/TESTING.md) (no PHI in test data). - **Completed: 2026-02-06**
...
- [x] ✅ 5.3 Type-check and lint pass; verify against [DEFINITION_OF_DONE.md](../../Reference/engineering/development/DEFINITION_OF_DONE.md) where applicable. - **Completed: 2026-02-06**
...
- [x] Unit test for resolution function; type-check and lint pass ([TESTING.md](../../Reference/engineering/development/TESTING.md)).
- [x] Responses and errors follow canonical contracts where applicable ([CONTRACTS.md](../../Reference/engineering/architecture/CONTRACTS.md)); no PHI in logs ([COMPLIANCE.md](../../Reference/engineering/compliance/COMPLIANCE.md)).
