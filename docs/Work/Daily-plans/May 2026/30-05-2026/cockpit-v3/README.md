...
> All phases for this program are **complete**. The live appointment-detail cockpit is `CockpitV3Shell` only.
...
**Deferred fast-follows** (capture inbox + product plan §Open questions): V3-Q1 default seed layout, per-(doctor × consult-type) persistence, preset CRUD UI port; legacy `templates.tsx` / `InvestigationsAutoMerge` glue removal once reference audit is green.
