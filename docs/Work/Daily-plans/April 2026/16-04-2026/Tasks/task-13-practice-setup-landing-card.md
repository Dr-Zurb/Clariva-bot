...
- [x] 4.1 Jest is still not wired up in the frontend (confirmed via `find frontend -name "jest.config*"` — zero matches; see `docs/Work/capture/inbox.md` 2026-04-04 entry carried forward from Task 07 / 12). `describeServicesCardState` is pure and intentionally structured for a future Jest pass — return shape is a plain object, no React, no async.
