...
- [x] All existing tests pass (including fee-path tests) — 32/32 matcher+deterministic, 73/73 consultation fees / quote / verification. Unrelated pre-existing failure in `webhook-worker-characterization.test.ts` remains captured in `docs/Work/capture/inbox.md`.
