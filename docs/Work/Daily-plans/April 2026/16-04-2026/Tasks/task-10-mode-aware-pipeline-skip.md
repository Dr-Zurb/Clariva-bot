...
- **Converting existing learning examples to `catalog_mode`-aware** — learning data from before Task 08 may lack the mode flag on the stored row; a separate data-migration is captured in `docs/Work/capture/inbox.md` if we want to clean that up. This task only affects new rows.
...
- Back-filling `catalog_mode` onto pre-Task-08 `service_match_learning_examples` rows — noted in `docs/Work/capture/inbox.md`.
