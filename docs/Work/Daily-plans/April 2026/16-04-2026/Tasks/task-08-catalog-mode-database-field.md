...
- **Frontend/backend enum parity** is maintained manually here (the same `['single_fee', 'multi_service']` literal exists in two files). Parity risk is small (2 strings) but captured in `docs/Work/capture/inbox.md` if a shared module is ever extracted.
