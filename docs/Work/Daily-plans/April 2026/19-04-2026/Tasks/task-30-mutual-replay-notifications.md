...
- [ ] **Frontend tests** (DEFERRED — the frontend repo doesn't yet ship a Jest/RTL harness; introducing one is its own task. Captured in `docs/Work/capture/inbox.md`. Manual smoke test below covers the v1 paths):
...
3. **Patient-side dashboard for replay events.** Patients don't have a clariva dashboard in v1; they receive DMs. If/when a patient portal ships, a symmetric feed would be the obvious move — captured in `docs/Work/capture/inbox.md` as a Plan 2.x concern.
...
2. **`backend/src/types/database.ts`** was **not** extended for `doctor_dashboard_events`. The dashboard-events-service helpers carry their own typed DTOs (`DashboardEventRow`, `InsertDashboardEventInput`) and no other module reaches into the table directly. Captured as a follow-up cleanup in `docs/Work/capture/inbox.md` for whenever a second caller materializes.
