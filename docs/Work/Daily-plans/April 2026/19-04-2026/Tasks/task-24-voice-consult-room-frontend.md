...
- **Resend-link rate limiting.** v1 ships unlimited resend (with a small "you've sent 3 in 5 minutes" inline soft warning above the buttons after the third call); proper backend rate-limiting (e.g. `consultation_resend_audit` table) is a follow-up captured in `docs/Work/capture/inbox.md`.
