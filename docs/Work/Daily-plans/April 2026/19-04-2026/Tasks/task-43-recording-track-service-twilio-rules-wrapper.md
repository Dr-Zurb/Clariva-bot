...
- [x] **Critical-log / alarm hook** — every `failed` ledger write is accompanied by a `logger.error` with the action + session id + original error. Captured in `docs/Work/capture/inbox.md` as a Plan 2.x follow-up to route `critical` logs to the on-call channel.
