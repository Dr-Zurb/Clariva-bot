...
- Add `appointments.consultation_modality` column + populate it during the booking flow (Plan 02 candidate). Captured in `docs/Work/capture/inbox.md`. The pre-consult cron is a no-op until this lands.
- Bootstrap a live-Supabase test harness with `auth.jwt()` shim so RLS policies can be tested end-to-end in CI. Captured in `docs/Work/capture/inbox.md`.
