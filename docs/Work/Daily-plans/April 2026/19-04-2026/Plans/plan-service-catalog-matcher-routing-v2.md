...
- Short **developer doc** (e.g. `docs/Work/...` or inline `README` under matcher): **Stage A** = fast substring/rule pass; if it returns, **Stage B is skipped**. **Stage B** = LLM chooses `service_key` from allowlist when Stage A returns null.
...
| # | Phase | Title | Notes |
|---|-------|--------|--------|
| [01](../Tasks/task-01-stage-a-stage-b-documentation.md) ✅ | 0 | Stage A / B doc + optional in-app explainer | `docs/Reference/product/learning/service-catalog-matching-stages.md` + drawer copy |
