...
`Drafted` — awaiting selection review. **Strongly recommended in full** because most items are already shipped on voice and the symmetric video implementation is cheap.
...
|---|------|--------|--------------|
| T1.1 | **Mute / unmute mic** — mic-off button in the controls bar; standard mic-slash icon when muted; companion-chat system message via Plan 06 `mute_changed` (sibling of voice T1.8 — already shipped). | XS (~30 min) | `VideoRoom.tsx` controls bar. |
| T1.2 | **Camera off / on** — separate button next to mute; turns OFF the local video track (replaces tile with avatar / initials placeholder); companion-chat system message `camera_changed` (new system_subtype). | S (~2h) | `VideoRoom.tsx` + `VideoSelfTile` placeholder. |
| T1.3 | **Call duration timer** in header (`mm:ss`, monotonic from `connected` event). Same hook as voice T1.1. | XS (~30 min) | `VideoRoom.tsx` header overlay. |
| T1.4 | **End-call confirmation** — "End the call? [Cancel / End]" modal. Doctor side bypass with shift-click for power users. (Sibling of voice T1.5 — reuse component verbatim.) | XS (~30 min) | Reuse `<EndCallConfirmModal>` from voice batch. |
| T1.5 | **Self-view position toggle** — own tile defaults to bottom-right corner overlay on remote tile (PiP-style); tap to flip corners (TL / TR / BL / BR); persisted per-device. | S (~3h) | `VideoRoom.tsx` layout + new `<VideoSelfTile>`. |
| T1.6 | **Self-view mirror toggle** — mirror own selfie video (default ON, matches every selfie camera UX); persisted per-device. | XS (~30 min) | `<VideoSelfTile>` CSS `transform: scaleX(-1)`. |
| T1.7 | **Pre-call camera + mic check** — one screen before connect: live selfie preview + mic level bar + camera dropdown + mic dropdown + "Continue / Skip mic check". | M (~5h) | New `VideoConsultPreCall.tsx`; route `/consult/join/page.tsx` mounts pre-call before VideoRoom. |
| T1.8 | **Network-quality 4-bar indicator** — Twilio `Participant.networkQualityLevel` 0–5 → 4-bar icon. Both sides. **Video extension:** detail tooltip shows `RTT / jitter / resolution / fps / kbps in / kbps out` (vs voice's lighter tooltip). | S (~3h) | `VideoRoom.tsx` overlay + extends shared `useNetworkQuality` hook. |
...
**Owner:** TBD
**Created:** 2026-04-29
**Status:** Drafted; **strongly recommended for first video batch in full** — closes the largest perceived-quality gap and most items are sibling re-mounts of voice work that's already shipped.
