...
- **DL-7: JWT role stays `receptionist`; desk permissions live on `clinic_staff.capabilities`.** Revised 2026-09-13. `app_metadata.role` remains `'receptionist'` so `/desk` routing does not change. The row holds `capabilities` (`front_desk` | `vitals` | `history` | `internal_labs` | `papers`) and `role` (`receptionist` if the login can register, else `assistant`). Payments are part of `front_desk`. Leftover `billing` folds into `front_desk`; leftover `previsit` folds into the four prep seats. **How those seats render:** parent R13–R17 — one `/desk`, job = panel, prep order filter-not-sort.

- **DL-8: Staff accounts are provisioned by script in this batch** (resolved RQ1). `backend/scripts/provision-clinic-staff.ts` and `/admin/clinic-staff` still work. **Doctor-facing invite landed 2026-08-23:** Settings → Staff. **Seats revised 2026-09-13:** one active login per ticked job (`237_staff_capability_seats.sql`). Existing auth accounts are never converted — only a new user, or an account already linked to that doctor.
