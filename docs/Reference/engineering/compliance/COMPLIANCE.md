...
**Source of Truth:** This document governs compliance posture, data handling, and security requirements for Clariva. `STANDARDS.md` governs code quality and patterns. If conflicts exist, `STANDARDS.md` remains the code source of truth, but `COMPLIANCE.md` defines system constraints that code must satisfy. Response-contract rules live in [STANDARDS.md](../development/STANDARDS.md) → "Canonical Contracts".
