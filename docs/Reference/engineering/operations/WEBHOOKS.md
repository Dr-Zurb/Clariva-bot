...
- [CONTRACTS.md](../architecture/CONTRACTS.md) - Idempotency contract and headers
- [RECIPES.md](../development/RECIPES.md) - Webhook implementation patterns
- [COMPLIANCE.md](../compliance/COMPLIANCE.md) - PHI handling and audit requirements
- [STANDARDS.md](../development/STANDARDS.md) - Logging rules and PII redaction
...
**Instagram POST — documented exceptions (RBH-08):** HMAC is always attempted first. If verification **fails**, `handleInstagramWebhook` may still return **200** for specific payload classes (e.g. some read/delivery-style payloads, `message_edit` no-queue path) or **continue processing** Instagram DMs/comments when product signing mismatches are observed. **Branch matrix, threat model, staging checklist:** [WEBHOOK_SECURITY.md](../compliance/WEBHOOK_SECURITY.md). **Razorpay** and **PayPal** webhooks have **no** equivalent bypass.
...
**Purpose:** Explain why `message_edit` is treated specially at the HTTP layer, why the worker still has `tryResolveSenderFromMessageEdit` + **`decodeMidExperimental`**, and when **not** to delete that code. Detailed incident notes: [Instagram DM reply troubleshooting](../Work/Daily-plans/February%202026/Week%203/instagram-dm-reply-troubleshooting.md).
...
**See:** [CONTRACTS.md](../architecture/CONTRACTS.md) "Idempotency Contract" section
...
- [Instagram DM reply troubleshooting](../Work/Daily-plans/February%202026/Week%203/instagram-dm-reply-troubleshooting.md) — historical experiments + debug checklist (RBH-11 points here for operator context)
- [WEBHOOK_SECURITY.md](../compliance/WEBHOOK_SECURITY.md) - Instagram signature bypass audit, threat model, re-test triggers (RBH-08)
- [CONTRACTS.md](../architecture/CONTRACTS.md) - Idempotency contract
- [RECIPES.md](../development/RECIPES.md) - Implementation patterns
- [COMPLIANCE.md](../compliance/COMPLIANCE.md) - PHI handling and audit
