...
| `webhook_message_edit_dropped_total` (RBH-11) | `alertMarker: rbh11_message_edit_dropped`, `provider`, `hasText`, `hasSender`, `hasMid`, `numEdit?` | Controller skipped queueing a message_edit-only webhook (deliberate — no edit reprocessing) |
| `dm_emergency_intent_downgraded_total` | `alertMarker: dm_emergency_intent_downgraded`, `reason: post_escalation_stability`, `correlationId` | Classifier returned `emergency` after a prior 112 in-thread, but post-policy downgraded to `medical_query` because the patient message showed **positive stability** evidence (keywords / non-crisis BP). No message text. |
| `dm_emergency_safety_decision_total` | `alertMarker: dm_emergency_safety_decision`, `regexHit`, `classifierIntent`, `emergencyGateEligible`, `emergencyGateFired`, `inCollection`, `priorEscalationInWindow`, `crisisOpen`, `headBranch` | One per DM turn (after head gates). Diagnose safety misses: `regexHit=true` + `emergencyGateFired=false` is abnormal unless `headBranch=revoke_consent`. `crisisOpen=true` + `emergencyGateFired=false` + `headBranch=null` should not happen after open-crisis inversion. No message text. |
| `dm_emergency_number_floor_applied_total` | `alertMarker: dm_emergency_number_floor_applied`, `branch`, `correlationId` | Outbound reply discussed emergency guidance without 112/108; floor appended localized escalation + opened `safety.escalatedAt`. Elevated rate → classifier/gate miss rate (LLM improvising crisis copy). No message text. |
| `dm_language_decision_total` (lang-18) | `alertMarker: dm_language_decision`, `storedBefore`, `resolved`, `changed`, `reason`, `hiMarkerCount`, `paMarkerCount`, `paExclusiveCount`, `accumulationWindowSize`, `classifierLanguage`, `classifierAgreed` | One per DM turn (including no-change). Counts and locale codes only — never message text or marker identities (LANG-D9). `classifierLanguage` / `classifierAgreed` are null until lang-17. |
...
- **message_edit silent-drop alert (RBH-11):** Over a rolling window (e.g. 15–30 min), if `webhook_message_edit_dropped_total` is elevated **and** Instagram `webhook_job_dequeued_total` / `webhook_instagram_dm_delivery_total` is near zero → Meta may be delivering real DMs as edit-only again (patient DMs ack’d 200 and never processed). Page on call; verify webhook field subscriptions. Separately, track share of drops with `numEdit` set — that estimates genuine patient edits (product signal; we still do not reprocess).
- **Post-112 emergency downgrade:** Count `dm_emergency_intent_downgraded_total` vs turns where `instagram_dm_routing.branch = emergency_safety` after a prior emergency in the same conversation. A sudden spike in downgrades without matching stability-style patient behaviour warrants review of `applyEmergencyIntentPostPolicy` / classifier stickiness.
- **Emergency safety decision miss:** Filter `dm_emergency_safety_decision_total` where `regexHit = true` and `emergencyGateFired = false`. Expected only when `headBranch = revoke_consent`. Any other combo → page / investigate (ratchet broken or stale process).
- **Emergency number floor rate:** Track `dm_emergency_number_floor_applied_total` vs `emergency_safety` branches. Occasional applications catch LLM improvisation; a sustained spike means bare `emergency` / classifier misses are common — revisit SAFE-D1 routing.
- **Open-crisis leak:** Filter where `crisisOpen = true` and `emergencyGateFired = false` and `headBranch` is null or a booking/medical branch — should be near-zero after the open-crisis gate (only stability resume + revoke are allowed overrides).
- **Language stuck on English:** Sustained `dm_language_decision_total` with `resolved = en` and `hiMarkerCount >= 1` across many turns → threshold or wordlist miss in production (LANG4). Page / review corpus fails.
- **Classifier vs markers drift (lang-17):** Rising `classifierAgreed = false` rate means the wordlist is drifting from reality — revisit markers; do not silence the classifier.

**Code:** `backend/src/services/webhook-metrics.ts` (helpers); `backend/src/workers/webhook-worker.ts` (BullMQ wrapper + Instagram branches); controller drop path in `backend/src/controllers/webhook-controller.ts`; emergency post-policy in `backend/src/services/ai-service.ts` + `run-conversation-turn.ts`; per-turn decision log in `handle-turn.ts`.
