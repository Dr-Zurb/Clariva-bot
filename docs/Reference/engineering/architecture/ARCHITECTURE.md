...
Use `successResponse(data, req)` and throw typed errors — never hand-roll `{ data }` or `{ error, message, stack }`, and always include `meta` (`timestamp`, `requestId`). **Canonical shape + rules live in [STANDARDS.md](../development/STANDARDS.md) → "Canonical Contracts"** (single source of truth; not duplicated here).
...
**See also:** [PATIENT_REGISTRATION_AND_ROSTER.md](../../product/patients-and-practice/PATIENT_REGISTRATION_AND_ROSTER.md) — early bot intake vs **registered** patient (MRN) and doctor **Patients** list visibility.
...
**Protected routes:** Routes that require authentication must mount `authenticateToken` (from `middleware/auth.ts`) before the controller. Optionally mount `userLimiter` after auth for per-user rate limiting. See [RECIPES.md](../development/RECIPES.md) R-AUTH-001.
...
- **[FRONTEND_STANDARDS.md](../development/FRONTEND_STANDARDS.md)** – Frontend coding rules (TypeScript, API consumption, a11y)
- **[FRONTEND_RECIPES.md](../development/FRONTEND_RECIPES.md)** – Copy-pastable frontend patterns (API client, Supabase, auth guard)
...
### Tier 1 (Must-Have):
- [STANDARDS.md](../development/STANDARDS.md) - Coding standards (single source of truth)
...
- [CODING_WORKFLOW.md](../development/CODING_WORKFLOW.md) - Step-by-step coding process
- [RECIPES.md](../development/RECIPES.md) - Code patterns
