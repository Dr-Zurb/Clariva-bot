...
const envSchema = z.object({
   ...
   * This is the project's **HS256 shared secret** (legacy signing scheme).
   * Two consumers:
   *   1. `services/supabase-jwt-mint.ts` (Plan 04 · Task 18) — signs scoped
   *      patient + doctor JWTs for the text-consult Realtime channel.
   *   2. `utils/supabase-token-verifier.ts` (np-02) — verifies the doctor's
   *      Supabase access token **locally** in the auth middleware, removing
   *      the per-request `getUser` round-trip. When this is unset the
   *      middleware safely falls back to the remote `getUser` check, so
   *      auth still works — it just doesn't get the latency win.
   * Optional at startup so non-text / pre-rollout deployments keep booting,
   ...
   * Default off — production dispatch unchanged until real creds + worker route exist.
   */
  WHATSAPP_ENABLED: z
    .string()
    .optional()
   ...
   * Default off in production until staged rollout (see EXECUTION-ORDER-p5-receptionist-returning-memory.md).
   * Dev flips to true after rcp-24 privacy gates pass.
   */
  RETURNING_PATIENT_MEMORY_ENABLED: z
    .string()
    .optional()
    .transform((v) => v === 'true' || v === '1'),
  ...
  /**
   * Web Push (task-text-D6a): VAPID keypair for browser push when PWA is
   * backgrounded. Optional at startup — push-notification-service no-ops when
   * unset. Generate via `npx web-push generate-vapid-keys`.
   */
  WEB_PUSH_VAPID_PUBLIC_KEY: z.string().optional(),
  WEB_PUSH_VAPID_PRIVATE_KEY: z.string().optional(),
  /** mailto: contact required by Web Push spec for upstream-provider abuse. */
  ...
  // EHR Sub-batch B2 / T3.16 — Prescription share-link HMAC.
  // Generate via `openssl rand -hex 32`. The token service throws at
  // mint/verify time when missing; the env validation stays optional
  // so `npm run dev` doesn't reject startup before the operator
  // provisions secrets in dev/staging/prod.
  // (The patient-facing base URL reuses the existing `APP_BASE_URL`
  // declared further below — see Plan 04 Task 18 entry.)
  RX_SHARE_TOKEN_SECRET: z
    .string()
    .min(32, 'RX_SHARE_TOKEN_SECRET must be at least 32 chars (use `openssl rand -hex 32`)')
    .optional(),
   ...
   * See `docs/Work/Architecture/legacy-appointment-fee-minor-deprecation.md`.
   */
  ...
  // ==========================================================================
  // Patient seeing flow · pf-17 — Auto-no-show worker
  // ==========================================================================
  /**
   * Master kill-switch for the in-process auto-no-show worker (pf-17).
   * When `true` (default in production), `startAutoNoShowWorker()` is
   * invoked from `index.ts` after the HTTP server starts. When `false`,
   * the worker logs a single `auto-no-show-worker: disabled by env`
   * line and never schedules a tick.
   *
   * Default behaviour:
   *   - unset / empty in production  → enabled (`true`)
   *   - unset / empty in dev or test → disabled (`false`)
   *   - explicit `'true'`            → force-enable (e.g. dev smoke)
   *   - explicit `'false'` / `'0'`   → force-disable
   *
   * Per pf-17 § Configuration: doctors must opt in via
   * `doctor_settings.auto_no_show_after_min` regardless of this flag —
   * the worker scans only rows where that column is non-NULL — so even
   * with this enabled, default behaviour is "no-op" for doctors who
   * haven't opted in (P-D7).
   */
  AUTO_NO_SHOW_WORKER_ENABLED: z
    .string()
    .optional()
    .transform((v) => {
  ...
  /**
   * OPD per-day mode (pdm-06): drains `doctor_opd_pending_mode_notifications`
   * every 60s. When unset, enabled in production only (mirrors pf-17).
   */
  OPD_MODE_NOTIFICATIONS_WORKER_ENABLED: z
    .string()
    .optional()
    .transform((v) => {
      if (v === undefined || v === '') return undefined;
  ...
  /**
   * OPD per-day mode (pdm-09): session overrun flagging (5 min) + 24h fallback (hourly).
   * When unset, enabled in production only.
   */
  OPD_OVERRUN_WORKER_ENABLED: z
    .string()
    .optional()
    .transform((v) => {
      if (v === undefined || v === '') return undefined;
  ...
  /**
   * Patient seeing flow · pf-17 § Wrap-up sweep (P-Q2). When `true`, the
   * same worker tick that scans for auto-no-show candidates also sweeps
   * appointments stuck in the "consult ended but doctor never opened the
   * wrap-up dialog" state for >24h, flipping them to `'completed'` with
   * a `appointment.auto_completed_wrap_up_stuck` audit row.
   *
   * Ships **dark** (default `false`) per pf-17 line 114 — the
   * forensic-trust contract on auto-completion is contentious; ops
   * flips this to `true` once the no-show path has been observed in
   * production for one full day without surprises.
   */
  AUTO_WRAP_UP_SWEEP_ENABLED: z
    .string()
    .default('false')
    .transform((v) => v === 'true' || v === '1'),
});
