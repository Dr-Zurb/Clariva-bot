/**
 * How investigations should print on the Rx. Twin of
 * frontend/lib/cockpit/investigations-rx-layout.ts — keep the rules in lockstep.
 */
...
function parseInvestigationSegments(value: string): string[] {
  return value
    .split(';')
    .map((part) => part.trim())
    .filter(Boolean);
...
function looksLikeInvestigationNote(text: string): boolean {
  ...
  if (!t) return false;
  if (/^(bring|come|please|take|return|keep|show|call|do not)\b/i.test(t)) {
    return true;
  }
...
export function layoutInvestigationsForRx(
  ...
  const segments = parseInvestigationSegments(trimmed);
