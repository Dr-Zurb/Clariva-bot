...
import { logSecurityEvent, drainAuditLogQueue } from './utils/audit-logger';
...
// Tailnet origins (clariva-dev.tail363099.ts.net + short MagicDNS name) added
// so the patient phone — joined to the same tailnet — can hit the local
// frontend at http://clariva-dev.tail363099.ts.net:3000 and have its API calls
// to the local backend (http://clariva-dev.tail363099.ts.net:3001) pass the
// preflight check. Safe to leave in dev because NODE_ENV=production swaps to
// `corsOptionsProd` which doesn't include these.
const corsOptionsDev: cors.CorsOptions = {
  origin: [
    // Laptop-only / direct-localhost dev.
    'http://localhost:3000',
    'http://localhost:3001',
    'http://127.0.0.1:3000',
    // Bare tailnet hostname (rare; only if hitting the dev server directly via
    // Tailscale node name without going through Funnel — kept for diagnostics).
    'http://clariva-dev:3000',
    'http://clariva-dev:3001',
    // Tailscale Funnel public origin. Frontend + backend are both behind this
    // single hostname via path routing, so most browser requests are actually
    // same-origin and don't trigger CORS — but listing it keeps preflights
    // happy when frontend is loaded from localhost while NEXT_PUBLIC_API_URL
    // points at the Funnel URL.
    'https://clariva-dev.tail363099.ts.net',
  ],
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'],
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Correlation-ID'],
...
// pf-17: in-process auto-no-show + wrap-up sweep worker handle (set after listen)
let autoNoShowWorker: AutoNoShowWorkerHandle | null = null;
// pdm-06: debounced OPD mode-change notification drainer (60s)
...
initializeDatabase()
  .then(() => {
    ...
    server = app.listen(PORT, () => {
      ...
      startWebhookWorker();
      // pf-17: in-process auto-no-show worker. Honours
      // AUTO_NO_SHOW_WORKER_ENABLED; per-doctor opt-in via
      // doctor_settings.auto_no_show_after_min keeps it inert by default.
      autoNoShowWorker = startAutoNoShowWorker();
      // pdm-06: drain debounced mode-change notification batches every 60s.
...
const gracefulShutdown = (signal: string) => {
  ...
  const shutdownAsync = async (): Promise<void> => {
    try {
      // pf-17: stop the auto-no-show worker first — it's a synchronous
      // clearInterval + flag flip, so it's the cheapest piece of cleanup
      // and ensures no new tick starts while we tear down the queue.
      if (autoNoShowWorker) {
        autoNoShowWorker.stop();
        autoNoShowWorker = null;
      }
      if (opdModeNotificationsWorker) {
        opdModeNotificationsWorker.stop();
        opdModeNotificationsWorker = null;
      }
      if (opdOverrunWorker) {
      ...
      await closeQueue();
      // np-03: flush pending audit rows after workers stop but before exit.
