/**
 * Consultation hold-notification service (voice T2.11 / task-voice-B3)
 *
 * Posts a `hold_changed` system row into the companion chat when a
 * participant puts the call on hold or resumes. Both doctor and patient
 * JWTs may call this route.
 *
 * @see backend/src/services/consultation-message-service.ts · emitHoldChanged
 */
...
import { emitHoldChanged } from './consultation-message-service';
...
export interface PostHoldChangedOptions {
  sessionId: string;
  bearerJwt: string;
  body: unknown;
...
export interface PostHoldChangedResult {
  onHold: boolean;
  emittedAt: string;
}

interface ResolvedHoldCaller {
  actorRole: 'doctor' | 'patient';
  actorId: string;
}

function validateBody(raw: unknown): { onHold: boolean; actorName?: string } {
  if (!raw || typeof raw !== 'object') {
    throw new ValidationError('Request body must be a JSON object');
  }
  const obj = raw as Record<string, unknown>;
  const onHold = obj.onHold;
  if (typeof onHold !== 'boolean') {
    throw new ValidationError('Field `onHold` is required and must be a boolean');
  }
  ...
  return { onHold, actorName };
}

async function resolveHoldCaller(
  sessionId: string,
  bearerJwt: string,
): Promise<ResolvedHoldCaller> {
  ...
  if (status !== 'live') {
    throw new ForbiddenError('Hold notifications are only allowed during a live session');
  }
  ...
  if (consultRole === 'extra_participant') {
    throw new ForbiddenError('Extra participants cannot post hold notifications');
  }
...
export async function postHoldChanged(
  options: PostHoldChangedOptions,
): Promise<PostHoldChangedResult> {
  ...
  const caller = await resolveHoldCaller(sessionId, bearerJwt);
  ...
  logger.info(
    ...
    'postHoldChanged: hold_changed banner emitted',
  );
