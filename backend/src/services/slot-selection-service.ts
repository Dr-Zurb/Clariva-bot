...
function inferSlotBookingCatalogQuoteBlockReason(
  ...
  const idRaw = state.serviceMatch?.catalogServiceId?.trim();
  if (idRaw && !findServiceOfferingByServiceId(catalog, idRaw)) {
    return 'invalid_catalog_service_id';
  }
  const raw = state.serviceMatch?.catalogServiceKey?.trim().toLowerCase();
...
export function resolveModalityForSlotBooking(
  ...
  const m = state.serviceMatch?.consultationModality;
  if (m === 'text' || m === 'voice' || m === 'video') {
    return m;
  }
  const ct = consultationType ?? state.booking?.consultationType;
...
export function applyPublicBookingSelectionsToState(
  ...
  if (state.booking?.consultationType === 'in_clinic') {
    return state;
  }
  const catalog = getActiveServiceCatalog(doctorSettings);
  if (!catalog) {
  ...
  let serviceKey =
    ...
    state.serviceMatch?.catalogServiceKey?.trim().toLowerCase() ??
    undefined;
  const idIn = input.catalogServiceId?.trim() ?? state.serviceMatch?.catalogServiceId?.trim();
  if (idIn) {
  ...
  let modality: ConsultationModality | undefined =
    input.consultationModality ?? state.serviceMatch?.consultationModality;
...
function appointmentConsultationTypeFromState(
  ...
  return resolveModalityForSlotBooking(state, state.booking?.consultationType);
}
...
export async function computeSlotBookingQuote(
  ...
  const modality = resolveModalityForSlotBooking(state, state.booking?.consultationType);
...
export async function processSlotSelectionAndPay(
  ...
  const sessionResolved =
    adminClient != null
      ? await resolveSessionDayMode(adminClient, doctorId, sessionDateYmd)
      : { mode: doctorSettings?.opd_mode === 'queue' ? ('queue' as const) : ('slot' as const) };
