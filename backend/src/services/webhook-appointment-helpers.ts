...
export function buildRelatedPatientIdsForWebhook(
  ...
  state: Pick<ConversationState, 'booking' | 'bookingForOther'>
): string[] {
  const ids = [conversationPatientId];
  if (state.booking?.lastBookingPatientId && state.booking?.lastBookingPatientId !== conversationPatientId) {
    ids.push(state.booking?.lastBookingPatientId);
  }
  if (state.bookingForOther?.bookingForPatientId && !ids.includes(state.bookingForOther?.bookingForPatientId)) {
    ids.push(state.bookingForOther?.bookingForPatientId);
  }
