 ...
 *
 * rcp-28 (per-doctor identity): the caller must pass the **per-doctor**
 * `patients.id` for the deleting clinic's row (from that doctor's
 * conversation / booking token). The scrub touches only that row by primary
 * key — it never glob-nulls `platform_external_id` across other doctors'
 * rows that share the same IG sender.
 *
