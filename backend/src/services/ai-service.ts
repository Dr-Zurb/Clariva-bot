...
export function redactPhiForAI(text: string): string {
  ...
  // Conservative catch-all: any run of >= 10 digits
  out = out.replace(/\b\d{10,}\b/g, '[REDACTED_PHONE]');
  // Collapse overlapping pattern matches
  out = out.replace(/(?:\[REDACTED_PHONE\])+/g, '[REDACTED_PHONE]');
  return out;
}
...
export interface ExtractionContext {
  ...
  /**
   * `true` when the patient is replying to the read-back summary at
   * `confirm_details`. Activates a stricter prompt addendum: only return
   * fields where the patient supplied a new value; complaints without a
   * value (e.g. `"wrong"`, `"got my name wrong"`) must yield `{}`.
   */
...
export async function extractFieldsWithAI(
  ...
  const isCorrection = context?.isCorrection === true;
  ...
  if (lastBotMessage && (missingFields.length > 0 || isCorrection)) {
  ...
  if (isCorrection) {
    // Plan: "bot copy bugs 2026-04-27" — at confirm_details the patient is
    // replying to the read-back summary. Without this addendum the model
    // tends to invent values for fields the patient never re-stated.
    parts.push(
      'CORRECTION MODE: The patient is replying to a read-back of details we already collected. ONLY extract a field when the patient gave a NEW value for it in this single message. Do NOT carry forward, infer, or repeat any field from the read-back. If the message is a complaint with no new value (e.g. "wrong", "you got my name wrong", "no my number is wrong", "that\'s not right"), return {} — that means "no new fields", which is correct. The handler will ask the patient for the right value next.'
    );
  } else {
    parts.push(
      `Extract only fields relevant to what we asked. If we asked for gender and they gave a long reply, extract only gender.`
    );
  }
...
Return JSON only.`;
  ...
  try {
    ...
    if (typeof parsed.name === 'string' && parsed.name.trim().length >= 2) {
      ...
      // these come from corrections like `name: "my email is X"`.
      const isOtherFieldPrefix =
        /^\s*(?:my|your)\s+(?:e[\s-]*mail|email|phone|mobile|number|age|gender|sex|reason|symptom)\b/i.test(
          name
        );
      if (!isComplaintSentinel && !isVerbStartingPhrase && !isOtherFieldPrefix) {
        result.name = name;
      }
    }
...
export function applyIntentPostClassificationPolicy(
  result: IntentDetectionResult,
  messageText: string,
  state: Pick<ConversationState, 'lastPromptKind' | 'triage'>
): IntentDetectionResult {
  if (result.intent !== 'book_appointment') return result;
  const feeThread =
    state.triage?.activeFlow === 'fee_quote' ||
    state.lastPromptKind === 'fee_quote' ||
    state.triage?.reasonFirstTriagePhase !== undefined;
...
// open-ended extraction there (see docs/Reference/product/receptionist-bot/AI_BOT_BUILDING_PHILOSOPHY.md).
// ============================================================================
...
export interface GenerateResponseContext {
  ...
  /**
   * rcp-21: PHI-safe returning-patient tone hint (opaque keys + enums only).
   * e.g. "returning patient: prior_visits=2, last_service=[follow_up], recency=[within_3_months]"
   */
  returningPatientSummary?: string;
}
...
${pricingGuardrails}`;
  ...
  if (
    ...
    prompt += `\n\nTELECONSULT-ONLY (product rule): This practice uses the **teleconsult catalog** above for visit types (text / voice / video only). Do **not** offer in-clinic or in-person appointments, do **not** quote a street address for booking, and do **not** invite users to visit the clinic physically—unless the Practice info block above explicitly states otherwise (it should not when this rule appears). If they ask for an address, say only: "Appointments are online, so there isn't a street address. I can help with timings or a booking link." Do not use the word teleconsult in that reply. Do **not** ask the user to choose between text, voice, or video in this chat — they will pick their consultation mode on the booking page when they select a slot.`;
  }
...
export async function generateResponseWithActions(
  ...
  const cancelContext =
    state.step === 'awaiting_cancel_confirmation' && state.cancel?.appointmentId
      ...
      : '';
  const pickContext =
    (state.step === 'awaiting_cancel_choice' && state.cancel?.pendingAppointmentIds?.length) ||
    (state.step === 'awaiting_reschedule_choice' && state.reschedule?.pendingAppointmentIds?.length)
