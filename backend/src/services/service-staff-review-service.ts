...
export async function upsertPendingStaffServiceReviewRequest(params: {
  ...
  const key = params.state.serviceMatch?.matcherProposedCatalogServiceKey?.trim();
  if (!key) {
    throw new InternalError('upsertPendingStaffServiceReviewRequest: missing matcherProposedCatalogServiceKey');
  }

  const confidence = params.state.serviceMatch?.serviceCatalogMatchConfidence ?? 'medium';
  const proposedId = params.state.serviceMatch?.matcherProposedCatalogServiceId?.trim();
  const modality = params.state.serviceMatch?.matcherProposedConsultationModality;
  ...
  const insertPayload = {
    ...
    match_reason_codes: asJsonArray(params.state.serviceMatch?.serviceCatalogMatchReasonCodes),
    candidate_labels: (() => {
      if (params.candidateLabels?.length) return params.candidateLabels;
      if (params.state.serviceMatch?.matcherCandidateLabels?.length) {
        return params.state.serviceMatch.matcherCandidateLabels;
      }
      return [];
    })(),
    ...
    for (const c of convs ?? []) {
      ...
      const meta =
        row.metadata != null && typeof row.metadata === 'object'
          ? readConversationState(row.metadata)
          : null;
      reasonMap.set(row.id, truncateReasonPreview(meta?.booking?.reasonForVisit));
    }
