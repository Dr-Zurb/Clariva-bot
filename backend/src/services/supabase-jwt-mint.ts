...
export type ConsultRole = 'doctor' | 'patient' | 'extra_participant';
...
export interface MintScopedJwtInput {
   ...
   *  - Extra participant (Sub-batch C · task-video-C8): a synthetic
   *    string `'extra:{participantId}'` where participantId is the
   *    `consultation_extra_participants.id` row that backs the invite.
   *    See `buildExtraParticipantSub()`.
   */
  sub: string;

  /** Whether this JWT is for a doctor, patient, or extra participant. Drives the custom claim. */
  role: ConsultRole;
  ...
  /**
   * For `role: 'extra_participant'` only — the
   * `consultation_extra_participants.id` row id. Pinned via the
   * `extra_participant_id` claim so the SELECT RLS branch on
   * `consultation_messages` (Migration 085) can scope the read
   * window to this specific invite row. Required when role is
   * 'extra_participant', ignored otherwise.
   */
  extraParticipantId?: string;
...
export interface ScopedJwtPayload {
  ...
  /**
   * Set ONLY when `consult_role === 'extra_participant'` — the row
   * id of the `consultation_extra_participants` invite that this
   * JWT was minted for. Mirrors the `extra_participant_id` claim
   * the SELECT RLS predicate (Migration 085) reads via
   * `auth.jwt() ->> 'extra_participant_id'`.
   */
  extra_participant_id?: string;
}
...
export function mintScopedConsultationJwt(
  ...
  if (input.role === 'extra_participant') {
    const extraId = input.extraParticipantId?.trim();
    if (!extraId) {
      throw new InternalError(
        "mintScopedConsultationJwt: extraParticipantId is required when role === 'extra_participant'",
      );
    }
    payload.extra_participant_id = extraId;
  }
...
export function verifyScopedConsultationJwt(token: string): ScopedJwtPayload {
  ...
  if (
    ...
    (p.consult_role !== 'doctor' &&
      p.consult_role !== 'patient' &&
      p.consult_role !== 'extra_participant')
  ) {
    throw new InternalError(
  ...
  if (
    p.consult_role === 'extra_participant' &&
    typeof p.extra_participant_id !== 'string'
  ) {
    throw new InternalError(
      "verifyScopedConsultationJwt: extra_participant token missing 'extra_participant_id' claim",
    );
  }
...
/**
 * Build the synthetic extra-participant `sub` from the
 * `consultation_extra_participants.id` row id. Used by the C8 invite
 * exchange endpoint when minting a Supabase JWT for the third party.
 *
 * The `extra:{participantId}` prefix matches the patient pattern so
 * `safe_uuid_sub()` (Migration 079) returns NULL — guaranteeing no
 * 22P02 cast failure if a future RLS predicate tries to coerce
 * `auth.jwt() ->> 'sub'` to UUID.
 */
export function buildExtraParticipantSub(extraParticipantId: string): string {
  const trimmed = extraParticipantId?.trim();
  if (!trimmed) {
    throw new InternalError(
      'buildExtraParticipantSub: extraParticipantId is required',
    );
  }
