 ...
 *
 * rcp-28 (per-doctor identity): acts only on the `patientId` passed in — the
 * per-doctor row referenced by `conversation.patient_id`. Platform identity
 * (`platform`, `platform_external_id`) is intentionally **not** cleared so the
 * same sender can re-contact this clinic and re-run consent on the same row.
 *
