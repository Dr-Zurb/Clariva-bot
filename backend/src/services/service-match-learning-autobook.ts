...
export async function tryApplyLearningPolicyAutobook(params: {
  ...
  if (
    ...
    params.state.serviceMatch?.pendingStaffServiceReview !== true ||
    !params.state.serviceMatch?.matcherProposedCatalogServiceKey?.trim()
  ) {
    return { applied: false };
  }

  const proposed = params.state.serviceMatch!.matcherProposedCatalogServiceKey!.trim().toLowerCase();
  const { patternKey, canonical } = buildPatternKeyFromInputs({
    matchReasonCodes: normalizeMatchReasonCodes(params.state.serviceMatch?.serviceCatalogMatchReasonCodes),
  ...
  let nextState = applyFinalCatalogServiceSelection(params.state, {
    ...
    consultationModality: params.state.serviceMatch?.matcherProposedConsultationModality ?? undefined,
    clearProposal: true,
