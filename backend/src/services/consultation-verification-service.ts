 ...
 *
 * Identity convention (e-task-3):
 *   - `doctor-{doctorId}`         → stamps `appointments.doctor_joined_at`
 *                                   AND mirrors join + status='live' onto
 *                                   the matching `consultation_sessions` row.
 *   - `patient-{appointmentId}`   → stamps `appointments.patient_joined_at`
 *                                   AND mirrors join + status='live' onto
 *                                   the matching `consultation_sessions` row.
 *
 * IMPORTANT — appointment-level vs session-level guards:
 *   `appointments.doctor_joined_at` / `patient_joined_at` are LEGACY
 *   first-write-wins fields used by the verification/payout pipeline. Once
 *   set on the FIRST consult of a given appointment they stay set, even
 *   across follow-up consults that re-use the same appointment row (when
 *   that pattern emerges) and especially across local dev re-tests.
 *
 *   `consultation_sessions.status` is NEW per session and ALWAYS starts at
 *   `'scheduled'` (see `persistSessionRow`). It MUST be promoted to
 *   `'live'` on every participant-connected webhook for the new session,
 *   otherwise the `consultation_messages_insert_live_participants` RLS
 *   policy (migration 078) will deny every user-side INSERT — chat send
 *   silently fails for both doctor AND patient and renders a "Retry"
 *   pill. System messages still appear because the backend writes those
 *   with the service-role admin client (RLS bypass).
 *
 *   Therefore: the session-level mirror call (`mirrorJoinEventToSession`)
 *   runs UNCONDITIONALLY once we've matched identity-to-appointment, while
 *   the appointment-level UPDATE keeps its `IS NULL` guard. Both inner
 *   helpers are individually idempotent (`markParticipantJoined` filters
 *   on `IS NULL`; `updateSessionStatus` is a flat status set).
 */
export async function handleParticipantConnected(
  ...
  if (identity.startsWith('doctor-')) {
    ...
    if (apt.doctor_id !== doctorId) {
      logger.warn(
        { correlationId, identity, appointmentId: apt.id, expectedDoctorId: apt.doctor_id },
        'participant-connected identity doctor_id mismatch — ignoring'
      );
      return;
    }

    if (!apt.doctor_joined_at) {
    ...
    // Always mirror to consultation_sessions — promotes status to 'live' so
    // user-side `consultation_messages` INSERTs pass RLS. See JSDoc above.
    ...
    if (apt.id !== appointmentIdFromIdentity) {
      logger.warn(
        { correlationId, identity, appointmentId: apt.id, identityAppointmentId: appointmentIdFromIdentity },
        'participant-connected identity appointment_id mismatch — ignoring'
      );
      return;
    }
    ...
    // voice-C3 · T5.32 — doctor-only Web Push when patient joins voice/video.
    if (resolved.modality === 'voice' || resolved.modality === 'video') {
      void sendPatientJoinedCallPushToDoctor({
        ...
        logger.warn(
          ...
          'Patient joined call — Web Push fan-out failed (non-fatal)',
        );
      });
    }
  }
