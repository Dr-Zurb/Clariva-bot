...
import type { ConversationState, PatientCollectionStep } from '../types/conversation';
...
const STEP_BY_FIELD: Record<PatientCollectionField, PatientCollectionStep> = {
 ...
 */
export function getInitialCollectionStep(): PatientCollectionStep {
  return 'collecting_all';
}

/**
 * e-task-dm-04: Pre-seed Redis `reason_for_visit` when `state.booking?.reasonForVisit` was set during reason-first triage.
...
export interface ValidateAndApplyExtractedOptions {
   ...
   * `confirm_details`. In that mode all required fields are already collected,
   * so the legacy gate `missingFields.length > 0` would skip the AI extractor
   * entirely and fall back to the loose regex heuristics — which used to mis-
   * parse `"my email - X@Y.com"` as `name`, and `"you got my name wrong"` as
   * `name = wrong` + `reason_for_visit = "you got my name wrong"`.
   *
   * When `isCorrection` is set:
   *   1. The regex pass uses `fastPathOnly: true` so loose name / reason
   *      heuristics never run; only labeled fields, phone, email, age, and
   *      gender are picked up.
   *   2. The AI extractor runs **regardless of `missingFields`** with a
   *      correction-aware prompt addendum telling it that a complaint
   *      without a new value (e.g. `"wrong"`, `"got my name wrong"`)
   *      contains zero fields and must return `{}`.
   */
...
export async function validateAndApplyExtracted(
  ...
  // In correction mode, the AI extractor must run even when no required field
  // is missing — that's the whole point of the read-back step. Without this,
  // the regex fallback alone decides field updates, and the heuristic path
  // is what produced the "my email - X@Y.com → name" and "got my name wrong
  // → name=wrong" bugs in production.
  const shouldTryAi =
    text.trim().length > 0 && (missingFields.length > 0 || isCorrection);
  ...
  if (shouldTryAi) {
    ...
    const extractionContext: Partial<ExtractionContext> = {
      ...
      missingFields,
      collectedSummary,
      relation: currentState.bookingForOther?.bookingForSomeoneElse
        ? currentState.bookingForOther?.relation
        : undefined,
    ...
    const aiExtracted = await extractFieldsWithAI(
      redactPhiForAI(text),
      // For corrections, hand the AI an empty `missingFields` list so it
      // doesn't feel pressured to invent values for fields the patient never
      // mentioned. The correction-prompt addendum (in `ai-service.ts`)
      // explicitly tells it to return only fields the patient gave NEW
      // values for.
      isCorrection ? [] : missingFields,
      correlationId,
      extractionContext
    );
    if (Object.keys(aiExtracted).length > 0) {
      ...
      // In correction mode, fall back to the SAFE regex (fastPathOnly) only.
      // The heuristic name/reason pass is what produced the production bugs
      // and must never run on a correction reply.
      const regexFallback = extractFieldsFromMessage(text, {
        fastPathOnly: isCorrection,
      }) as Partial<CollectedPatientData>;
    ...
    const regexResult = extractFieldsFromMessage(text, {
      fastPathOnly: isCorrection,
    }) as Partial<CollectedPatientData>;
