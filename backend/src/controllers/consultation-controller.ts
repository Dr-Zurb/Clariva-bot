...
import {
  mintAttachmentSignedUrls,
  textSessionSupabaseAdapter,
} from '../services/text-session-supabase';
import { submitSnapshot } from '../services/snapshot-storage-service';
import { postQuickActionBanner } from '../services/consultation-quick-actions-service';
import { postAutoFallbackBanner } from '../services/consultation-auto-fallback-service';
import { postMuteChanged } from '../services/consultation-mute-service';
import { postHoldChanged } from '../services/consultation-hold-service';
import { ingestVideoQualitySamples } from '../services/video-call-quality-service';
import { ingestVoiceQualitySamples } from '../services/voice-call-quality-service';
import { getPostCallSummary } from '../services/post-call-summary-service';
import {
  attachSnapshotToSection,
  discardSnapshot,
  isClinicalSection,
  listSnapshots,
} from '../services/snapshot-review-service';
import {
  createInvite as createExtraParticipantInvite,
  exchangeInviteToken as exchangeExtraParticipantInvite,
  listInvitesForSession as listExtraParticipantInvites,
  recordParticipantLeft as recordExtraParticipantLeft,
  revokeInvite as revokeExtraParticipantInvite,
} from '../services/consultation-extra-participant-service';

// ============================================================================
// Companion-capable modalities allow-list (Plan 06 Decision 9 / voice-0A)
//
// `exchangeTextConsultTokenHandler` is the patient-side mint path for the
// companion chat channel. The chat channel ships on every voice / video
// consult (alongside the call channel) AND on text-only consults, so the
// allow-list covers all three. Any future modality (e.g. an async
// chat-only-without-realtime mode) deliberately falls through to the
// 400-rejection branch — adding it here is an explicit, audited decision.
// ============================================================================
...
export const exchangeTextConsultTokenHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (
      !COMPANION_CAPABLE_MODALITIES.includes(
        session.modality as CompanionCapableModality,
      )
    ) {
      // Error message intentionally preserved verbatim ("Cannot exchange
      // text-token for {modality} session") so any Sentry rule / log
      // alert that grepped on the prior text keeps firing for genuinely
    ...
    if (session.status !== 'ended' && session.status !== 'cancelled') {
      // This endpoint's contract is "mint the chat-channel token",
      // regardless of the session modality. Voice/video sessions still
      // need a Supabase JWT for their companion chat (Plan 06 Decision
      // 9), but `facadeGetJoinToken` would dispatch by `session.modality`
      // and return a Twilio access token for voice/video — wrong shape
      // entirely for the chat layer. Calling the text adapter directly
      // pins the contract: chat-channel token, no matter the call
      // modality. Do NOT "fix" this back to the facade.
      //
      // We pass the adapter input that the facade would have synthesized
      // (it re-reads the row internally; we already have it in scope, so
...
// ============================================================================
// Plan 06 · attachment signed-URL minting
//
// Why a backend route instead of `client.storage.createSignedUrl()` from
// the chat UI: the patient JWT carries a synthetic `sub`
// (`patient:{appointmentId}`) which the storage-api auth layer doesn't
// always honor the same way PostgREST does, even though the
// `storage.objects` RLS policy (migration 079) green-lights the patient
// branch. Routing through service-role here is the bulletproof fix —
// session membership is re-enforced in `mintAttachmentSignedUrls` from
// the same JWT claims the storage RLS would key on, so the security
// surface is unchanged.
//
// POST /api/v1/consultation/:sessionId/attachments/sign
// Auth: Bearer JWT (doctor's Supabase auth JWT OR our patient-scoped
//       JWT minted by `mintScopedConsultationJwt`). The handler
//       verifies-and-decides which branch applies; no separate
//       middleware needed.
// Body: { paths: string[] } — storage object keys, each must start
//       with `${sessionId}/`. Bad paths are silently dropped.
// Returns: { urls: { [path]: signedUrl } } — only successfully-minted
//       paths appear in the map.
// ============================================================================

export const mintAttachmentSignedUrlsHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    if (!body || !Array.isArray(body.paths)) {
      throw new ValidationError('Body { paths: string[] } is required');
    }
    const paths: string[] = body.paths.filter(
      (p): p is string => typeof p === 'string',
    );

    const result = await mintAttachmentSignedUrls({
      sessionId,
      paths,
      bearerJwt,
      correlationId,
...
//
// POST /api/v1/consultation/:sessionId/snapshots
//
// Auth — same shape as `mintAttachmentSignedUrlsHandler`: a Bearer JWT
//        in the Authorization header, branched by `consult_role` claim
//        (doctor's Supabase auth JWT OR our scoped patient JWT). The
//        service does the verification + session-membership re-check.
//
// Body — JSON envelope (NOT multipart). The original task draft asked
//        for multipart/form-data; we ship JSON+base64 instead because:
//
//          1. No new dependency. Multipart parsing requires `multer`
//             or similar; JSON+base64 fits the existing
//             `express.json()` body parser at a 10 MB cap.
//          2. PHI hygiene — the JPEG round-trip stays in one Buffer
//             (decode base64 → service buffer → storage upload), no
//             temp-file landing on the disk.
//          3. v1 snapshots are <1 MB at 1080p; the ~33% base64 overhead
//             keeps the payload comfortably inside the 10 MB body cap.
//
//        Multipart can be added additively later (the service's input
//        already takes a `Buffer`; the handler is the only thing that
//        couples to the wire shape).
//
//        Required fields:
//          - `jpegBase64`:   base64-encoded JPEG bytes (no data: prefix).
//          - `target`:       'self' | 'remote' (visibility metadata key).
//          - `dimensions`:   { width: number, height: number } in px.
//
// Returns — { snapshotId, url, attachmentPath } per the service contract.
//
// Errors —
//   400 ValidationError    — bad payload (missing fields, oversized,
//                            non-JPEG bytes, dimensions out of range)
//   401 UnauthorizedError  — bad / missing / mismatched bearer
//   403 ForbiddenError     — patient lacks recording consent
//   404 NotFoundError      — session row missing
//   500 InternalError      — storage / DB failure
// ============================================================================

const SNAPSHOT_BASE64_REGEX = /^[A-Za-z0-9+/=\r\n]+$/;

export const postSnapshotHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const body = req.body as
      | {
          ...
          target?: unknown;
          dimensions?: unknown;
          // Sub-batch C · task-video-C4 — optional annotation overlay
          // metadata. The JPEG payload is already composited (annotations
          // burned into the raster); this field is the structured
          // companion record. Strict shape validation lives in the
          // service layer (`validateAnnotations`) so the contract is
          // unified across this route and any future caller.
          annotations?: unknown;
        }
      | undefined;
    ...
    if (typeof jpegBase64 !== 'string' || jpegBase64.length === 0) {
      throw new ValidationError('jpegBase64 must be a non-empty string');
    }
    // Defensive shape check — keeps obvious garbage out before we
    // call Buffer.from() (which silently produces garbage on bad
    // input rather than throwing).
    if (!SNAPSHOT_BASE64_REGEX.test(jpegBase64)) {
      throw new ValidationError('jpegBase64 contains non-base64 characters');
    }

    const target = body.target;
    if (target !== 'self' && target !== 'remote') {
      throw new ValidationError("target must be 'self' or 'remote'");
    }
    ...
    if (
      ...
      throw new ValidationError('dimensions { width, height } must be numbers');
    }

    let jpegBytes: Buffer;
    try {
      jpegBytes = Buffer.from(jpegBase64, 'base64');
    } catch (err) {
      throw new ValidationError(
        `jpegBase64 decode failed: ${err instanceof Error ? err.message : 'unknown'}`,
      );
    }

    // Sub-batch C · task-video-C4 — pass annotations through opaquely;
    // `submitSnapshot` runs `validateAnnotations` on whatever shape the
    // client sends, so we don't double-validate at the route boundary.
    // Distinguish "field omitted" (undefined → no annotation work) from
    // "field present but malformed" (passed through → validator throws
    // ValidationError with the precise field path).
    const annotations =
      body.annotations === undefined ? undefined : body.annotations;
...
// POST /api/v1/consultation/:sessionId/quick-action-banner
//
// Doctor-only. Posts a `'rx_sent'` or `'follow_up_scheduled'` system row
...
// underlying clinical action (Rx send via `sendPrescriptionToPatient`,
// appointment create via `createAppointment`) is performed by the
// frontend through its existing dashboard API helpers — this endpoint
// only exists to mint the in-channel banner row that the existing flows
// don't know about.
//
// Request body:
//   { kind: 'rx_sent', prescriptionId: string }
// or:
//   { kind: 'follow_up_scheduled', appointmentId: string, scheduledAt: ISO }
//
// Auth: doctor Supabase JWT (patient JWTs are 403'd at the service gate).
//
// Response: { kind, emittedAt }
//
// @see backend/src/services/consultation-quick-actions-service.ts
// @see docs/Work/Daily-plans/April 2026/28-04-2026/Tasks/task-video-C6-in-call-quick-actions.md
// ============================================================================

export const postQuickActionBannerHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    // Pass the body through opaquely to the service-layer validator —
    // narrow `unknown` -> typed action there so the validation rules
    // are co-located (same shape `postSnapshotHandler` uses for
    // annotations).
...
// POST /api/v1/consultation/:sessionId/auto-fallback-banner
//
// Doctor-only. Posts an `auto_audio_fallback` (engaged) or
// `auto_audio_recovered` (restored) system row into the consultation
// chat after the local <VideoRoom>'s adaptive controller has either
// engaged audio-only fallback (E1's "very low" floor breach) or the
// doctor has clicked "Try video again" to restore video.
//
// The on-screen <AudioFallbackBanner> over the video canvas is the
// local immediate UI; this row is the in-channel transcript record
// that survives recording / replay AND is visible to the patient via
// their existing chat companion (so they understand WHY the camera
// went dark — the A2 camera-off avatar alone doesn't disambiguate
// "manual camera toggle" from "auto-fallback because of bandwidth").
//
// Request body (one of):
//   { kind: 'engaged',  attempt: number, thresholdLevel: number }
...
// Response: { kind, emittedAt }
//
// @see backend/src/services/consultation-auto-fallback-service.ts
...
// @see docs/Work/Daily-plans/April 2026/28-04-2026/Tasks/task-video-E2-auto-audio-fallback.md
// ============================================================================

export const postAutoFallbackBannerHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
...
// ============================================================================
// Sub-batch A · voice T1.8 / task-voice-A7 — counterparty mute notification
// POST /api/v1/consultation/:sessionId/mute-changed
//
// Doctor OR patient companion JWT. Body: { muted: boolean, actorName?: string }.
// Emits a `mute_changed` system row visible to both parties via Realtime.
//
// @see backend/src/services/consultation-mute-service.ts
// ============================================================================

export const postMuteChangedHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const result = await postMuteChanged({
      sessionId,
      bearerJwt,
      body: req.body,
      correlationId,
...
// ============================================================================
// Sub-batch B · voice T2.11 / task-voice-B3 — hold / resume notification
// POST /api/v1/consultation/:sessionId/hold-changed
//
// Doctor OR patient companion JWT. Body: { onHold: boolean, actorName?: string }.
// Emits a `hold_changed` system row visible to both parties via Realtime.
//
// @see backend/src/services/consultation-hold-service.ts
// ============================================================================

export const postHoldChangedHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const result = await postHoldChanged({
      sessionId,
      bearerJwt,
      body: req.body,
      correlationId,
...
//   `backend/src/services/video-call-quality-service.ts` for the
//   `VideoQualitySample` shape (mirrors the column names in
//   `video_call_quality` table — Migration 086).
//
// Auth: doctor Supabase JWT OR patient companion JWT (the Supabase JWT
//   minted by the text-token exchange — same JWT the patient already
//   uses for the chat companion). The service layer routes on the
//   `consult_role` claim shape.
//
// Response: { inserted: number, sessionId: string, role: 'doctor' | 'patient' | 'extra_participant' }
//
// Fire-and-forget from the frontend reporter. Failures bubble as 500
// so the reporter retries on the next 60s flush; the call itself is
// never affected by a metrics POST failure (the reporter's `.catch`
// handler swallows network errors silently — see
// `frontend/lib/video/quality-reporter.ts`).
//
// @see backend/migrations/086_video_call_quality.sql
// @see backend/src/services/video-call-quality-service.ts
// @see frontend/lib/video/quality-reporter.ts (the caller)
// @see docs/Work/Daily-plans/April 2026/28-04-2026/Tasks/task-video-E6-qos-health-metrics.md
// ============================================================================

export const postVideoQualityHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const result = await ingestVideoQualitySamples({
      sessionId,
      bearerJwt,
      body: req.body,
      correlationId,
...
//
// Endpoint: POST /api/v1/consultation/:sessionId/voice-quality
//
// Auth: doctor Supabase JWT OR patient companion JWT (the Supabase JWT
//   minted by the text-token exchange — same JWT the patient already
//   uses for the chat companion). The service layer routes on the
...
// Response: { inserted: number, sessionId: string, role: 'doctor' | 'patient' | 'extra_participant' }
//
// Mirrors the video-quality handler one-for-one (Migration 086 / video
// E6); voice samples just have fewer columns (no resolution / fps /
// kbps). Fire-and-forget from the frontend reporter — failures bubble
// as 500 so the reporter retries on the next 60s flush; the call
// itself is never affected by a metrics POST failure.
//
// @see backend/migrations/105_voice_call_quality.sql
// @see backend/src/services/voice-call-quality-service.ts
// @see frontend/lib/voice/quality-reporter.ts (the caller)
// @see docs/Work/Daily-plans/April 2026/28-04-2026/Tasks/voice/task-voice-C2-qos-health-metrics.md
// ============================================================================

export const postVoiceQualityHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const result = await ingestVoiceQualitySamples({
      sessionId,
      bearerJwt,
      body: req.body,
      correlationId,
...
//
// Four endpoints round out the third-party invite lifecycle:
//
//   POST   /api/v1/consultation/:sessionId/extra-participants
//          → createExtraParticipantInviteHandler
//          Doctor-only. Body: { displayName, roleLabel? }. Returns
//          { participantId, inviteToken, inviteUrl, invitedAt }.
//
//   POST   /api/v1/consultation/extra-participant-exchange
//          → exchangeExtraParticipantInviteHandler
//          PUBLIC (no JWT). Body: { inviteToken }. Returns the
//          short-lived Supabase JWT + Twilio access token + the
//          room name to `connect()` to.
//
//   POST   /api/v1/consultation/:sessionId/extra-participants/:participantId/revoke
//          → revokeExtraParticipantInviteHandler
//          Doctor-only. Stamps `revoked_at` (and `left_at` if the
//          third party had joined).
//
//   GET    /api/v1/consultation/:sessionId/extra-participants
//          → listExtraParticipantInvitesHandler
//          Doctor-only. Returns the doctor's panel list.
//
//   POST   /api/v1/consultation/extra-participants/leave
//          → recordExtraParticipantLeftHandler
//          Extra-participant-JWT-only. Stamps `left_at` + emits
//          `participant_left` system banner. Idempotent.
//
...
export const createExtraParticipantInviteHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    if (typeof body.displayName !== 'string') {
      throw new ValidationError('Field `displayName` is required and must be a string');
    }
    if (
    ...
    const result = await createExtraParticipantInvite({
      sessionId,
      bearerJwt,
      displayName: body.displayName,
      roleLabel: (body.roleLabel as string | null | undefined) ?? null,
      correlationId,
    });
...
export const exchangeExtraParticipantInviteHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const body = (req.body ?? {}) as { inviteToken?: unknown };
    if (typeof body.inviteToken !== 'string') {
      throw new ValidationError('Field `inviteToken` is required and must be a string');
    }

    const result = await exchangeExtraParticipantInvite({
      inviteToken: body.inviteToken,
      correlationId,
    });

    // 200 (not 201) — the resource (extra-participant row) was
    // created by the doctor's POST; this endpoint only updates
    // (`joined_at`) and mints tokens.
    res.status(200).json(successResponse(result, req));
  },
);

export const revokeExtraParticipantInviteHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const params = req.params as { sessionId?: string; participantId?: string };
    ...
    if (!participantId) {
      throw new ValidationError('participantId path param is required');
    }
    ...
    const result = await revokeExtraParticipantInvite({
      sessionId,
      bearerJwt,
      participantId,
      correlationId,
...
export const listExtraParticipantInvitesHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const rows = await listExtraParticipantInvites({
      sessionId,
      bearerJwt,
      correlationId,
    });

    res.status(200).json(successResponse({ invites: rows }, req));
  },
);

export const recordExtraParticipantLeftHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    ...
    const result = await recordExtraParticipantLeft({
      bearerJwt,
      correlationId,
    });
 ...
 * @see docs/Work/Daily-plans/April 2026/19-04-2026/Tasks/task-31-post-consult-chat-history-surface.md
 */
...
// ============================================================================
// Sub-batch D · task-video-D1 — post-call summary aggregator.
//
//   GET    /api/v1/consultation/:sessionId/post-call-summary
//          → getPostCallSummaryHandler
//
// Auth: Bearer JWT — accepts EITHER a doctor's Supabase JWT OR a
// scoped patient/extra_participant JWT whose `session_id` claim
// matches the path-param. The service layer (`resolveCaller` in
// `post-call-summary-service.ts`) does the discrimination.
//
// Returns: PostCallSummaryDto (modality / status / duration /
// counterparty / attachments + snapshots counts / Rx-sent + id /
// recording status).
//
// @see backend/src/services/post-call-summary-service.ts
// @see docs/Work/Daily-plans/April 2026/28-04-2026/Tasks/task-video-D1-post-call-summary.md
// ============================================================================

export const getPostCallSummaryHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    const summary = await getPostCallSummary({
      sessionId,
      bearerJwt,
      correlationId,
    });
...
// ============================================================================
// Sub-batch D · task-video-D3 — snapshot review-and-attach handlers
//
// Three doctor-only endpoints under the existing consultation router:
//
//   GET    /api/v1/consultation/:sessionId/snapshots
//          → listSnapshotsHandler — gallery feed for <SnapshotReviewPanel>.
//
//   POST   /api/v1/consultation/:sessionId/snapshots/:snapshotId/attach-to-section
//          → attachSnapshotToSectionHandler — assigns a canonical
//            SOAP-style section to the snapshot row's metadata.
//
//   POST   /api/v1/consultation/:sessionId/snapshots/:snapshotId/discard
//          → discardSnapshotHandler — soft-deletes via metadata.discarded_at.
//
// Auth — doctor's Supabase JWT only. The service layer
// (`snapshot-review-service.ts#resolveDoctorForSession`) rejects any
// non-doctor caller, so we don't need a separate auth gate at the
// route level. Why not register this on the existing
// `authenticateToken` middleware? Two reasons:
//
//   1. The neighboring snapshot-capture route (POST snapshots) does
//      its own auth at the service boundary (because patients are
//      allowed to capture their own tile and they carry our scoped
//      JWT, not a real Supabase user JWT). Keeping the review path
//      symmetrical with capture means one auth model per resource.
//
//   2. Doctor-only is enforced inside the service, not at the
//      middleware, so there's only one source of truth on who-can-do-
//      what. If a future patient self-serve "discard my own snapshot"
//      affordance lands, we relax in one place.
// ============================================================================

export const listSnapshotsHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const sessionId = (req.params as { sessionId?: string }).sessionId?.trim();
    if (!sessionId) {
    ...
    // Default `includeDiscarded=true` so the doctor can see soft-
    // discarded rows + (in a future v2) restore them. Frontend can
    // pass `?includeDiscarded=false` to hide the discarded subset
    // for the gallery render path.
    const includeDiscardedRaw = (req.query.includeDiscarded ?? '') as string;
    const includeDiscarded =
      includeDiscardedRaw === ''
        ? true
        : includeDiscardedRaw === 'true' || includeDiscardedRaw === '1';
...
export const attachSnapshotToSectionHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    ...
    if (!snapshotId) {
      throw new ValidationError('snapshotId path param is required');
    }
    ...
    if (!isClinicalSection(body.section)) {
      throw new ValidationError(
        "section must be 'Subjective' | 'Objective' | 'Assessment' | 'Plan' | 'Attachments'",
      );
    }
    ...
    const updated = await attachSnapshotToSection({
      sessionId,
      snapshotId,
    ...
    res.status(200).json(successResponse(updated, req));
  },
);

export const discardSnapshotHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const correlationId = req.correlationId || 'unknown';
    const params = req.params as {
      sessionId?: string;
    ...
    const updated = await discardSnapshot({
      sessionId,
      snapshotId,
      bearerJwt,
      correlationId,
