 ...
 *
 * **`buildDmTurnSignals`** is the single assembly point (`feeCatalogMatchText` + `recentMedicalDeflection`).
 * The Instagram DM handler still calls **`buildFeeCatalogMatchText`** inline at several branches; a future
 * refactor should hoist one `buildDmTurnSignals(text, recentMessages, state)` per turn to avoid drift.
...
export interface DmTurnSignals {
...
export function buildDmTurnSignals(
  text: string,
...
/** @deprecated Use `buildDmTurnSignals` — disambiguated from engine `DmTurnContext` in rcp-12. */
export const buildDmTurnContext = buildDmTurnSignals;

/** @deprecated Use `DmTurnSignals`. */
export type DmTurnContext = DmTurnSignals;
