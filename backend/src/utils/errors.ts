...
/**
 * Advisory-lock acquisition timeout (409). Used by the OPD per-day mode
 * conversion service (pdm-04) when another conversion is in flight for
 * the same `(doctor_id, session_date)` pair and the application-level
 * mutex / future `pg_advisory_xact_lock` cannot be acquired before the
