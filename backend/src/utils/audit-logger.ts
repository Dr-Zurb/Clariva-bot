...
// ============================================================================
// Async emit queue (np-03 — off the request hot path, lossless)
// ============================================================================

/** Max in-memory backlog before producers slow down (never silently drop). */
const MAX_QUEUE_SIZE = 10_000;
/** Rows per PostgREST insert during a flush tick. */
const BATCH_SIZE = 50;

const pendingQueue: InsertAuditLog[] = [];
const backpressureWaiters: Array<() => void> = [];
let flushScheduled = false;
let isProcessing = false;
/** Set during graceful shutdown — disables backpressure cap and drains all pending rows. */
let isDraining = false;

function notifyBackpressureWaiters(): void {
  while (backpressureWaiters.length > 0 && pendingQueue.length < MAX_QUEUE_SIZE) {
    backpressureWaiters.shift()?.();
  }
}

async function waitForQueueSpace(): Promise<void> {
  while (pendingQueue.length >= MAX_QUEUE_SIZE && !isDraining) {
...
async function insertBatch(batch: InsertAuditLog[]): Promise<void> {
  ...
  const { error } = await supabaseAdmin.from('audit_logs').insert(batch);
  if (error) {
    logger.error({ error, count: batch.length }, 'Failed to flush audit log batch');
  }
}
...
async function processQueue(): Promise<void> {
  ...
  try {
    while (pendingQueue.length > 0) {
      const batchSize = isDraining
        ? pendingQueue.length
        : Math.min(BATCH_SIZE, pendingQueue.length);
...
async function enqueueAuditLog(auditLogData: InsertAuditLog): Promise<void> {
  await waitForQueueSpace();
  pendingQueue.push(auditLogData);
  scheduleFlush();
}

/**
 * Flush all pending audit rows before process exit (np-03).
 *
 * Call from the server bootstrap's SIGTERM/SIGINT handler after
 * `server.close()` so in-flight handlers can finish enqueueing but no new
 * requests arrive. Blocks until the queue is empty and the current flush
 * completes — nothing in flight is lost.
 */
export async function drainAuditLogQueue(): Promise<void> {
  isDraining = true;
...
/** @internal Test-only visibility into backlog depth. */
export function getPendingAuditLogCountForTests(): number {
  return pendingQueue.length;
}

/** @internal Reset queue state between tests. */
export function resetAuditLogQueueForTests(): void {
 ...
 *
 * **Emit path (np-03):** validation + enqueue are synchronous; the DB
 * insert runs on a background flush tick (batched). Callers resolve once
 * the row is **enqueued** — not once it is persisted — so the request hot
 * path is not blocked on PostgREST. Use `void logAuditEvent(...)` on latency-
 * critical paths (e.g. auth middleware) to skip even the enqueue backpressure
 * wait. `drainAuditLogQueue()` flushes pending rows on graceful shutdown.
 *
...
export async function logAuditEvent(params: {
  ...
  try {
    await enqueueAuditLog(auditLogData);
  } catch (error) {
    logger.error(
