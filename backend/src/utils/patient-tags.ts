/**
 * Normalize / apply clinic patient tags (patients-multi-tag / PMT-D3).
 * Not PHI — operational labels only.
 */
...
/** Dedupe case-insensitively; keep first-seen casing; cap at PATIENT_TAGS_MAX. */
...
/** Prefer array; fall back to legacy single patient_tag. */
...
/**
 * Labels in `tagsToAdd` that are new to `current` but dropped by the max-8 cap.
 */
export function unappliedAddLabels(
  current: readonly string[],
  tagsToAdd: readonly string[]
): string[] {
  const before = new Set(normalizeTagList([...current]).map((t) => t.toLowerCase()));
  const next = new Set(applyTagOp(current, 'add', tagsToAdd).map((t) => t.toLowerCase()));
...
/** Mirror first tag into legacy column during dual-write window. */
export function legacyPatientTagFromTags(tags: readonly string[]): string | null {
  return tags[0] ?? null;
}
