 ...
 *
 * IMPORTANT (see docs/Reference/product/receptionist-bot/AI_BOT_BUILDING_PHILOSOPHY.md §3–4.1):
 ...
 * `docs/Work/Daily-plans/April 2026/13-04-2026/planning/rt-04-triage-fees-dm-context-findings-and-planned-changes.md` §5.
 */
...
export function shouldDeferIdleFeeForReasonFirstTriage(params: {
  ...
  if (state.triage?.reasonFirstTriagePhase) return false;
  // Fees already shown (activeFlow === 'fee_quote') — never re-enter triage (migration 046 era fix).
  if (state.triage?.activeFlow === 'fee_quote') return false;
  ...
  if (isPricingInquiryMessage(tNorm)) {
    if (!state.triage?.postMedicalConsultFeeAckSent && isVagueConsultationPaymentExistenceQuestion(text)) {
      return false;
    }
    return true;
  }
 ...
 * Caller should not defer if {@link TriageState.reasonFirstTriagePhase} is already active (handled by the triage branch).
 */
export function bookingShouldDeferToReasonFirstTriage(params: {
  state: Pick<ConversationState, 'booking' | 'triage'>;
  text: string;
  recentMessages: { sender_type: string; content: string }[];
}): boolean {
  if (params.state.triage?.reasonFirstTriagePhase) return false;
  if (params.state.booking?.reasonForVisit?.trim()) return false;
  return (
