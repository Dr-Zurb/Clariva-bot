...
export function deriveSlotPageBookingHints(state: ConversationState): SlotPageBookingHints {
  ...
  if (sm?.serviceSelectionFinalized !== true) {
    return {};
  }
  const key = sm.catalogServiceKey?.trim().toLowerCase();
  if (!key) {
    return {};
  }
  ...
  const id = sm.catalogServiceId?.trim();
  if (id) {
    hints.suggestedCatalogServiceId = id;
  }

  const mod = sm.consultationModality;
  if (mod === 'text' || mod === 'voice' || mod === 'video') {
    hints.suggestedConsultationModality = mod;
  }

  const conf = sm.serviceCatalogMatchConfidence;
