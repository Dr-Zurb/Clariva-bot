...
describe('Consent Service', () => {
  ...
  describe('handleRevocation', () => {
    ...
    describe('rcp-28 per-doctor lifecycle', () => {
      it('retains platform identity on revoke for same-clinic re-consent', async () => {
        mockedPatient.findPatientById.mockResolvedValue({
          id: patientId,
          consent_status: 'granted',
          platform: 'instagram',
          platform_external_id: 'psid-shared-999',
        } as any);
        mockedPatient.updatePatient.mockResolvedValue({} as any);
        ...
        const updatePayload = mockedPatient.updatePatient.mock.calls[0]?.[1] as Record<
          ...
          unknown
        >;
        expect(updatePayload.consent_status).toBe('revoked');
        expect(updatePayload).not.toHaveProperty('platform');
        expect(updatePayload).not.toHaveProperty('platform_external_id');
      });

      it('revoke under Dr A patient row does not update Dr B row', async () => {
        const patientDrA = 'pat-doctor-a';
        const patientDrB = 'pat-doctor-b';
        mockedPatient.findPatientById.mockResolvedValue({
          id: patientDrA,
          consent_status: 'granted',
        } as any);
        ...
        expect(mockedPatient.updatePatient).toHaveBeenCalledWith(
          patientDrA,
          expect.objectContaining({ consent_status: 'revoked' }),
          correlationId
        );
        expect(mockedPatient.updatePatient).not.toHaveBeenCalledWith(
          patientDrB,
          expect.anything(),
          expect.anything()
        );
      ...
      it('re-contact after revoke sees revoked status on same per-doctor row', async () => {
        mockedPatient.findPatientById.mockResolvedValue({
          id: patientId,
          consent_status: 'revoked',
          platform: 'instagram',
