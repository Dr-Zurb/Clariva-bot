...
describe('AI Service', () => {
  ...
  describe('redactPhiForAI — India formats', () => {
    const indianPhones = [
      '9876543210',
      '98765 43210',
      '98765-43210',
    ...
    it.each(indianPhones)('redacts Indian mobile: %s', (input) => {
      const result = redactPhiForAI(input);
      expect(result).toBe('[REDACTED_PHONE]');
      expect(result).not.toContain('98765');
      expect(result).not.toContain('43210');
    });

    it('does not double-redact overlapping phone patterns', () => {
      expect(redactPhiForAI('9876543210')).not.toContain(
        '[REDACTED_PHONE][REDACTED_PHONE]'
      );
      expect(redactPhiForAI('+91 98765 43210')).not.toContain(
        '[REDACTED_PHONE][REDACTED_PHONE]'
      );
    });

    const nonPhoneNumbers = ['I am 25', 'option 2', '2 pm'];

    it.each(nonPhoneNumbers)('does not redact non-phone numbers: %s', (input) => {
      expect(redactPhiForAI(input)).toBe(input);
    });

    it('still redacts emails alongside Indian phones', () => {
      expect(redactPhiForAI('me@x.com')).toBe('[REDACTED_EMAIL]');
      expect(redactPhiForAI('call 98765 43210 or me@x.com')).toBe(
        'call [REDACTED_PHONE] or [REDACTED_EMAIL]'
      );
    });
  });
