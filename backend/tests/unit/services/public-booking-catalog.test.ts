...
describe('applyPublicBookingSelectionsToState', () => {
  ...
  it('skips catalog merge for in_clinic', () => {
    ...
    expect(next.serviceMatch?.catalogServiceKey).toBeUndefined();
  });
  ...
  it('auto-picks single service and single modality', () => {
    ...
    expect(next.serviceMatch?.catalogServiceKey).toBe('only');
  ...
  it('accepts valid service + modality', () => {
    ...
    expect(next.serviceMatch?.catalogServiceKey).toBe('skin');
    expect(next.serviceMatch?.catalogServiceId).toBe(id('skin'));
    expect(next.serviceMatch?.consultationModality).toBe('text');
  });
