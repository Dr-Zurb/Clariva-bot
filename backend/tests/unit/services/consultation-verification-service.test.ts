...
const sendPatientJoinedCallPushToDoctor = jest.fn().mockResolvedValue(undefined);

jest.mock('../../../src/services/voice-remote-join-push-service', () => ({
  sendPatientJoinedCallPushToDoctor: (...args: unknown[]) =>
...
describe('Consultation Verification Service', () => {
  ...
  describe('handleParticipantConnected', () => {
    ...
    it('sets patient_joined_at when patient connects', async () => {
      ...
      expect(sendPatientJoinedCallPushToDoctor).toHaveBeenCalledWith({
        sessionId: 'sess-1',
        doctorId: 'doc-1',
        modality: 'video',
        correlationId,
    ...
    it('sends Web Push when patient joins a voice session', async () => {
      findSessionByProviderSessionId.mockResolvedValueOnce({
        id: 'sess-voice',
        appointmentId: 'apt-1',
        doctorId: 'doc-1',
        patientId: null,
    ...
    it('does not Web Push when the doctor joins', async () => {
      const apt = {
        id: 'apt-1',
        doctor_id: 'doc-1',
        doctor_joined_at: null,
      ...
      expect(sendPatientJoinedCallPushToDoctor).not.toHaveBeenCalled();
    });
