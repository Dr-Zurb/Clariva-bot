...
function buildAdminMock(existing: ExistingPatient | null, opts?: {
  ...
  const capturedUpdateEq: Array<[string, string]> = [];
  ...
  const from = jest.fn((...args: unknown[]) => {
    ...
    const updateEq = jest.fn((col: string, val: string) => {
      capturedUpdateEq.push([col, val]);
...
describe('scrubPatientPiiFromLogs', () => {
  ...
  it('rcp-28: scrubs only the requested per-doctor patient row (isolation by primary key)', async () => {
    const mock = buildAdminMock({
      id: 'pat-doctor-a',
      email: 'a@example.com',
      date_of_birth: null,
      ...
      platform_external_id: 'ig-shared-psid',
    });
    mockedDb.getSupabaseAdminClient.mockReturnValue(mock.client);
    ...
    expect(mock.capturedUpdate).toHaveLength(1);
    expect(mock.capturedUpdate[0]?.platform_external_id).toBeNull();
    expect(mock.capturedUpdateEq).toEqual([['id', 'pat-doctor-a']]);
  });
});
