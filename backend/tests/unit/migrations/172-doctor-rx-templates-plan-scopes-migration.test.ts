/**
 * Content-sanity test for migration 172 (doctor_rx_templates plan_json +
 * Plan scope widen).
 */
...
const MIGRATION_PATH = resolve(
  __dirname,
  '../../../migrations/172_doctor_rx_templates_plan_scopes.sql',
);

describe('172_doctor_rx_templates_plan_scopes.sql', () => {
  const sql = readFileSync(MIGRATION_PATH, 'utf8');

  describe('plan_json payload column', () => {
    ...
    it('drops then re-adds the jsonb_typeof object CHECK (idempotent re-run)', () => {
      ...
      expect(sql).toMatch(
        /ADD CONSTRAINT\s+doctor_rx_templates_plan_json_is_object\s+CHECK \(jsonb_typeof\(plan_json\) = 'object'\)/,
      );
    });
  });
  ...
  describe('scope CHECK widen', () => {
    ...
    it('preserves prior subjective + objective + result + inv + meds scopes', () => {
      for (const scope of [
        'subjective_full',
        'chief_complaints',
        'past_medical',
    ...
    it('adds the Plan section + whole-tab scopes', () => {
      for (const scope of [
        'advice',
        'follow_up',
        'referral',
  ...
  describe('additive + reversible', () => {
    it('does not rewrite data or alter/drop existing columns', () => {
      ...
      // Rollback docs may mention DROP COLUMN IF EXISTS plan_json.
      expect(sql).not.toMatch(/DROP COLUMN(?!\s+IF EXISTS plan_json)/i);
    });

    it('documents a rollback path', () => {
      expect(sql).toMatch(/Rollback/i);
      expect(sql).toMatch(/DROP COLUMN IF EXISTS plan_json/);
    });
  });
