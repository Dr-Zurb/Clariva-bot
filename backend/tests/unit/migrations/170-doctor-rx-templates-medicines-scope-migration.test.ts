/**
 * Content-sanity test for migration 170 (doctor_rx_templates scope widen with
 * medicines).
 */
...
const MIGRATION_PATH = resolve(
  __dirname,
  '../../../migrations/170_doctor_rx_templates_medicines_scope.sql',
);

describe('170_doctor_rx_templates_medicines_scope.sql', () => {
  ...
  describe('scope CHECK widen', () => {
    ...
    it('preserves prior subjective + objective + result + investigations scopes', () => {
      for (const scope of [
        'subjective_full',
        'chief_complaints',
        'past_medical',
    ...
    it('adds the medicines scope', () => {
      expect(sql).toContain(`'medicines'`);
    });
  });
