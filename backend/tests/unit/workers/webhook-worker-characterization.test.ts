...
import * as patientIdentityService from '../../../src/services/patient-identity-service';
...
jest.mock('../../../src/services/patient-identity-service', () => ({
  resolvePatientForChannelSender: jest.fn(),
}));
...
/** Final persisted state from the single end-of-turn sink (rcp-01). */
function lastPersistedState(): Record<string, unknown> {
  const calls = jest.mocked(conversationService.updateConversationState).mock.calls;
...
describe('RBH-02 webhook worker characterization', () => {
  beforeEach(() => {
    ...
    jest.mocked(patientIdentityService.resolvePatientForChannelSender).mockResolvedValue({
      id: TEST_PATIENT_ID,
    } as never);
  ...
  describe('DM: awaiting_match_confirmation', () => {
    ...
    it('reply 2 uses second match id when two pending', async () => {
      ...
      expect(conversationService.updateConversationState).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('rcp-01: single persist per turn (DL-11)', () => {
    it('persists exactly once for idle greeting turn', async () => {
      jest.mocked(aiService.classifyIntent).mockResolvedValue({ intent: 'greeting', confidence: 1 } as never);

      await processWebhookJob(
        fakeJob({
          eventId: 'evt-rcp01-greeting',
          provider: 'instagram',
          payload: dmPayload('hello'),
          correlationId: 'corr-rcp01-greeting',
        })
      );
    ...
    it('persists exactly once for receptionist paused handoff', async () => {
      jest.mocked(doctorSettingsService.getDoctorSettings).mockResolvedValue({
        timezone: 'UTC',
        practice_name: 'Test Clinic',
        instagram_receptionist_paused: true,
      ...
      await processWebhookJob(
        fakeJob({
          eventId: 'evt-rcp01-paused',
          provider: 'instagram',
          payload: dmPayload('book please'),
          correlationId: 'corr-rcp01-paused',
        })
      );
    ...
    it('persists exactly once for in-flow consent re-prompt (step stays consent)', async () => {
      jest.mocked(conversationService.getConversationState).mockResolvedValue({
        step: 'consent',
        bookingForSomeoneElse: false,
        collectedFields: [],
      ...
      await processWebhookJob(
        fakeJob({
          eventId: 'evt-rcp01-consent',
          provider: 'instagram',
          payload: dmPayload('maybe'),
          correlationId: 'corr-rcp01-consent',
        })
      );
    ...
    it('rcp-22: returning consented book skips demographics — single persist, field names only in metadata', async () => {
      ...
      jest.mocked(appointmentService.listAppointmentsForPatient).mockResolvedValue([
        {
          ...
          doctor_id: TEST_DOCTOR_ID,
          patient_id: TEST_PATIENT_ID,
          appointment_date: new Date(Date.now() - 86400000 * 30),
          status: 'completed',
      ...
      await processWebhookJob(
        fakeJob({
          eventId: 'evt-rcp22-returning-book',
          provider: 'instagram',
          payload: dmPayload('I would like to book an appointment'),
          correlationId: 'corr-rcp22-returning',
        })
      );
      ...
      expect(persisted).toEqual(
        expect.objectContaining({
          step: 'collecting_all',
          collectedFields: expect.arrayContaining(['name', 'phone']),
        })
      );
      expect(persisted.collectedFields).not.toContain('reason_for_visit');
      expect(JSON.stringify(persisted)).not.toMatch(/Priya|\\+919876543210/);
      expect(aiService.generateResponse).not.toHaveBeenCalled();
      expect(mockSendMessage).toHaveBeenCalledWith(
        '987654321012345',
        expect.stringMatching(/reason for visit/i),
        'corr-rcp22-returning',
        'doctor-token'
      );
