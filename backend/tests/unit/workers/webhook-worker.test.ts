...
// notification-service transitively imports @react-pdf/renderer (ESM) via prescription-pdf-service;
// mock it so the suite loads under jest's CJS transform (mirrors webhook-worker-characterization).
...
describe('Webhook Worker', () => {
  ...
  describe('3.3 Worker processing errors', () => {
    ...
    it('unknown page (no doctor linked): markWebhookFailed, audit failure, optional fallback reply, no conversation', async () => {
      ...
      expect(mockMarkProcessed).not.toHaveBeenCalled();
      expect(patientIdentityService.resolvePatientForChannelSender).not.toHaveBeenCalled();
    });
  });
