...
import { buildDmTurnSignals, buildFeeCatalogMatchText } from '../../../src/utils/dm-turn-context';
...
describe('dm-turn-context', () => {
  ...
  describe('buildDmTurnSignals', () => {
    it('sets recentMedicalDeflection from state TTL', () => {
      const now = buildDmTurnSignals(
        'hi',
        [],
        readConversationState({ lastMedicalDeflectionAt: new Date().toISOString() })
      );
      expect(now.recentMedicalDeflection).toBe(true);

      const stale = buildDmTurnSignals(
        'hi',
        [],
