...
describe('validatePatchDoctorSettings (e-task-6)', () => {
  ...
  it('accepts cockpit_template_override for each R-MOD template id (tmr-03)', () => {
    for (const template of [
      ...
      const result = validatePatchDoctorSettings({ cockpit_template_override: template });
      expect(result.cockpit_template_override).toBe(template);
    }
  });

  it('rejects invalid cockpit_template_override (tmr-03)', () => {
    expect(() =>
      validatePatchDoctorSettings({ cockpit_template_override: 'invalid' }),
    ).toThrow(ValidationError);
  });

  it('accepts null cockpit_template_override to clear pin (tmr-03)', () => {
    const result = validatePatchDoctorSettings({ cockpit_template_override: null });
