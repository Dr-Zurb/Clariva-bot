/**
 * Cron route tests for POST /cron/ghost-account-sweep (auth-v2 · Model C).
 */
...
jest.mock('../../../src/workers/ghost-account-sweep-cron', () => ({
  runGhostAccountSweepJob: (...a: unknown[]) => mockRunJob(...a),
}));
...
function getHandler(): (req: Request, res: Response) => Promise<unknown> {
  ...
  const layer = stack.find(
    (l) =>
      l.route?.path === '/ghost-account-sweep' &&
      l.route.methods.post === true,
  );
  if (!layer?.route?.stack[0]?.handle) {
    throw new Error('POST /ghost-account-sweep handler not found');
  }
...
function makeReq(headers: Record<string, string>): Request {
  return {
    headers,
    method: 'POST',
    url: '/cron/ghost-account-sweep',
  } as unknown as Request;
}

describe('POST /cron/ghost-account-sweep', () => {
  beforeEach(() => {
    jest.clearAllMocks();
